 ...
 * (for RCU).
 */
#define _rcu_for_each_node_breadth_first(sp, rnp) for ((rnp) = &(sp)->node[0]; (rnp) < &(sp)->node[rcu_num_nodes]; (rnp)++)
#define rcu_for_each_node_breadth_first(rnp) _rcu_for_each_node_breadth_first (&rcu_state, rnp)
#define srcu_for_each_node_breadth_first(ssp, rnp) _rcu_for_each_node_breadth_first (ssp->srcu_sup, rnp)
...
#define for_each_leaf_node_possible_cpu(rnp, cpu)                                                                                 \
    for (WARN_ON_ONCE (!rcu_is_leaf_node (rnp)), (cpu) = cpumask_next ((rnp)->grplo - 1, cpu_possible_mask); (cpu) <= rnp->grphi; \
...
#define for_each_leaf_node_cpu_mask(rnp, cpu, mask)                                                                 \
    for (WARN_ON_ONCE (!rcu_is_leaf_node (rnp)), (cpu) = rcu_find_next_bit ((rnp), 0, (mask)); (cpu) <= rnp->grphi; \
...
static inline bool rcu_async_should_hurry (void)
{
    return false;
}
...
static inline bool rcu_cpu_online (int cpu)
{
    return true;
}
...
#else
static inline unsigned long rcu_get_jiffies_lazy_flush (void)
{
    return 0;
}
...
static inline void srcutorture_get_gp_data (struct srcu_struct* sp, int* flags, unsigned long* gp_seq)
{
    *flags  = 0;
    *gp_seq = sp->srcu_idx;
}
...
void srcutorture_get_gp_data (struct srcu_struct* sp, int* flags, unsigned long* gp_seq);

#endif

#ifdef CONFIG_TINY_RCU
static inline bool rcu_watching_zero_in_eqs (int cpu, int* vp)
{
    return false;
}
...
static inline bool rcu_check_boost_fail (unsigned long gp_state, int* cpup)
{
    return true;
}
...
#ifdef CONFIG_TINY_RCU
static inline bool rcu_cpu_beenfullyonline (int cpu)
{
    return true;
}
#else
