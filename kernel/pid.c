...
struct task_struct* pid_task (struct pid* pid, enum pid_type type)
{
    ...
    if (pid) {
        ...
        first = rcu_dereference_check (hlist_first_rcu (&pid->tasks[type]), lockdep_tasklist_lock_is_held ());
        if (first)
...
void __init pid_idr_init (void)
{
    ...
    init_pid_ns.pid_cachep
            = kmem_cache_create ("pid", struct_size_t (struct pid, numbers, 1), __alignof__ (struct pid), SLAB_HWCACHE_ALIGN | SLAB_PANIC | SLAB_ACCOUNT, NULL);
}
