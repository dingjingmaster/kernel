...
static void __mutex_remove_waiter (struct mutex* lock, struct mutex_waiter* waiter)
{
...
static inline bool ww_mutex_spin_on_owner (struct mutex* lock, struct ww_acquire_ctx* ww_ctx, struct mutex_waiter* waiter)
{
    struct ww_mutex* ww;
 ...
 */
static __always_inline int __sched __mutex_lock_common (struct mutex* lock, unsigned int state, unsigned int subclass, struct lockdep_map* nest_lock,
                                                        unsigned long ip, struct ww_acquire_ctx* ww_ctx, const bool use_ww_ctx)
{
...
static int __sched __mutex_lock (struct mutex* lock, unsigned int state, unsigned int subclass, struct lockdep_map* nest_lock, unsigned long ip)
{
    return __mutex_lock_common (lock, state, subclass, nest_lock, ip, NULL, false);
}

static int __sched __ww_mutex_lock (struct mutex* lock, unsigned int state, unsigned int subclass, unsigned long ip, struct ww_acquire_ctx* ww_ctx)
{
...
int __sched mutex_lock_killable_nested (struct mutex* lock, unsigned int subclass)
{
    return __mutex_lock (lock, TASK_KILLABLE, subclass, NULL, _RET_IP_);
}
EXPORT_SYMBOL_GPL (mutex_lock_killable_nested);

int __sched mutex_lock_interruptible_nested (struct mutex* lock, unsigned int subclass)
{
...
static inline int ww_mutex_deadlock_injection (struct ww_mutex* lock, struct ww_acquire_ctx* ctx)
{
...
int __sched ww_mutex_lock (struct ww_mutex* lock, struct ww_acquire_ctx* ctx)
{
    ...
    might_sleep ();
    ret = __ww_mutex_lock (&lock->base, TASK_UNINTERRUPTIBLE, 0, _RET_IP_, ctx);
...
int __sched ww_mutex_lock_interruptible (struct ww_mutex* lock, struct ww_acquire_ctx* ctx)
{
    ...
    might_sleep ();
    ret = __ww_mutex_lock (&lock->base, TASK_INTERRUPTIBLE, 0, _RET_IP_, ctx);
 ...
 * scheduler.
 * 像 mutex_lock() 那样锁定互斥锁。当任务正在等待这个互斥锁时，调度器将把它视为处于IO等待状态。
 *
 * Context: Process context.
 */
...
{
    return __ww_mutex_lock (&lock->base, TASK_UNINTERRUPTIBLE, 0, _RET_IP_, ctx);
}

static noinline int __sched __ww_mutex_lock_interruptible_slowpath (struct ww_mutex* lock, struct ww_acquire_ctx* ctx)
{
    return __ww_mutex_lock (&lock->base, TASK_INTERRUPTIBLE, 0, _RET_IP_, ctx);
}
