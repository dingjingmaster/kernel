 ...
 */
static struct sigqueue* __sigqueue_alloc (int sig, struct task_struct* t, gfp_t gfp_flags, int override_rlimit, const unsigned int sigqueue_flags)
{
...
still_pending:
        ...
        *resched_timer = (first->flags & SIGQUEUE_PREALLOC) && (info->si_code == SI_TIMER) && (info->si_sys_private);
...
static bool kill_ok_by_cred (struct task_struct* t)
{
    ...
    return uid_eq (cred->euid, tcred->suid) || uid_eq (cred->euid, tcred->uid) || uid_eq (cred->uid, tcred->suid) || uid_eq (cred->uid, tcred->uid)
...
static void complete_signal (int sig, struct task_struct* p, enum pid_type type)
{
	 ...
	 */
    if (sig_fatal (p, sig) && (signal->core_state || !(signal->flags & SIGNAL_GROUP_EXIT)) && !sigismember (&t->real_blocked, sig)
...
static int kill_pid_info_type (int sig, struct kernel_siginfo* info, struct pid* pid, enum pid_type type)
{
    int                 error = -ESRCH;
...
int force_sig_seccomp (int syscall, int reason, bool force_coredump)
{
    ...
    return force_sig_info_to_task (&info, current, force_coredump ? HANDLER_EXIT : HANDLER_CURRENT);
}
...
void do_notify_pidfd (struct task_struct* task)
{
    ...
    __wake_up (&pid->wait_pidfd, TASK_NORMAL, 0, poll_to_key (EPOLLIN | EPOLLRDNORM));
}
 ...
 */
static int ptrace_stop (int exit_code, int why, unsigned long message, kernel_siginfo_t* info) __releases (&current->sighand->siglock)
...
static void hide_si_addr_tag_bits (struct ksignal* ksig)
{
    ...
    case SIL_FAULT_BNDERR:
    case SIL_FAULT_PKUERR:
    case SIL_FAULT_PERF_EVENT: ksig->info.si_addr = arch_untagged_si_addr (ksig->info.si_addr, ksig->sig, ksig->info.si_code); break;
    case SIL_KILL:
    case SIL_TIMER:
    case SIL_POLL:
    case SIL_CHLD:
...
int sigprocmask (int how, sigset_t* set, sigset_t* oldset)
{
    ...
    switch (how) {
    case SIG_BLOCK: sigorsets (&newset, &tsk->blocked, set); break;
    case SIG_UNBLOCK: sigandnsets (&newset, &tsk->blocked, set); break;
    case SIG_SETMASK: newset = *set; break;
...
#ifdef CONFIG_COMPAT
int set_compat_user_sigmask (const compat_sigset_t __user* umask, size_t sigsetsize)
{
    sigset_t kmask;
...
static const struct
{
    unsigned char limit, layout;
        ...
        [SIGILL] = {NSIGILL, SIL_FAULT},  [SIGFPE] = {NSIGFPE, SIL_FAULT},   [SIGSEGV] = {NSIGSEGV, SIL_FAULT},
        [SIGBUS] = {NSIGBUS, SIL_FAULT},  [SIGTRAP] = {NSIGTRAP, SIL_FAULT},
#if defined(SIGEMT)
        [SIGEMT] = {NSIGEMT, SIL_FAULT},
#endif
        [SIGCHLD] = {NSIGCHLD, SIL_CHLD}, [SIGPOLL] = {NSIGPOLL, SIL_POLL},  [SIGSYS] = {NSIGSYS, SIL_SYS},
};
...
void copy_siginfo_to_external32 (struct compat_siginfo* to, const struct kernel_siginfo* from)
{
    ...
    case SIL_POLL:
        ...
        to->si_fd   = from->si_fd;
        break;
    case SIL_FAULT: to->si_addr = ptr_to_compat (from->si_addr); break;
    case SIL_FAULT_TRAPNO:
...
int __copy_siginfo_to_user32 (struct compat_siginfo __user* to, const struct kernel_siginfo* from)
{
...
static int post_copy_siginfo_from_user32 (kernel_siginfo_t* to, const struct compat_siginfo* from)
{
    ...
    case SIL_POLL:
        ...
        to->si_fd   = from->si_fd;
        break;
    case SIL_FAULT: to->si_addr = compat_ptr (from->si_addr); break;
    case SIL_FAULT_TRAPNO:
...
static int do_sigtimedwait (const sigset_t* which, kernel_siginfo_t* info, const struct timespec64* ts)
{
    ...
    if (!sig && timeout) {
        ...
        __set_current_state (TASK_INTERRUPTIBLE | TASK_FREEZABLE);
        ret = schedule_hrtimeout_range (to, tsk->timer_slack_ns, HRTIMER_MODE_REL);
        spin_lock_irq (&tsk->sighand->siglock);
...
static int copy_siginfo_from_user_any (kernel_siginfo_t* kinfo, siginfo_t __user* info)
{
    ...
    if (in_compat_syscall ())
        return copy_siginfo_from_user32 (kinfo, (struct compat_siginfo __user*)info);
#endif
...
#define PIDFD_SEND_SIGNAL_FLAGS (PIDFD_SIGNAL_THREAD | PIDFD_SIGNAL_THREAD_GROUP | PIDFD_SIGNAL_PROCESS_GROUP)
...
static inline void sigaltstack_unlock (void) __releases (&current->sighand->siglock)
{
    spin_unlock_irq (&current->sighand->siglock);
}
#else
...
SYSCALL_DEFINE2 (sigaltstack, const stack_t __user*, uss, stack_t __user*, uoss)
{
    ...
    if (uss && copy_from_user (&new, uss, sizeof (stack_t)))
        return -EFAULT;
    err = do_sigaltstack (uss ? &new : NULL, uoss ? &old : NULL, current_user_stack_pointer (), MINSIGSTKSZ);
...
static int do_compat_sigaltstack (const compat_stack_t __user* uss_ptr, compat_stack_t __user* uoss_ptr)
{
    ...
    ret = do_sigaltstack (uss_ptr ? &uss : NULL, &uoss, compat_user_stack_pointer (), COMPAT_MINSIGSTKSZ);
    if (ret >= 0 && uoss_ptr) {
        compat_stack_t old;
...
int __compat_save_altstack (compat_stack_t __user* uss, unsigned long sp)
{
    int                 err;
    struct task_struct* t = current;
    err                   = __put_user (ptr_to_compat ((void __user*)t->sas_ss_sp), &uss->ss_sp) | __put_user (t->sas_ss_flags, &uss->ss_flags)
...
SYSCALL_DEFINE3 (sigprocmask, int, how, old_sigset_t __user*, nset, old_sigset_t __user*, oset)
{
    ...
    if (nset) {
        ...
        switch (how) {
        case SIG_BLOCK: sigaddsetmask (&new_blocked, new_set); break;
        case SIG_UNBLOCK: sigdelsetmask (&new_blocked, new_set); break;
        case SIG_SETMASK: new_blocked.sig[0] = new_set; break;
        default: return -EINVAL;
        }
...
#    ifdef CONFIG_COMPAT
COMPAT_SYSCALL_DEFINE4 (rt_sigaction, int, sig, const struct compat_sigaction __user*, act, struct compat_sigaction __user*, oact, compat_size_t, sigsetsize)
{
...
#        endif
    ...
    if (!ret && oact) {
        ...
        ret |= put_compat_sigset (&oact->sa_mask, &old_ka.sa.sa_mask, sizeof (oact->sa_mask));
...
SYSCALL_DEFINE3 (sigaction, int, sig, const struct old_sigaction __user*, act, struct old_sigaction __user*, oact)
{
    ...
    if (act) {
        old_sigset_t mask;
        if (!access_ok (act, sizeof (*act)) || __get_user (new_ka.sa.sa_handler, &act->sa_handler) || __get_user (new_ka.sa.sa_restorer, &act->sa_restorer)
            || __get_user (new_ka.sa.sa_flags, &act->sa_flags) || __get_user (mask, &act->sa_mask))
            return -EFAULT;
...
#    endif
    ...
    if (!ret && oact) {
        if (!access_ok (oact, sizeof (*oact)) || __put_user (old_ka.sa.sa_handler, &oact->sa_handler) || __put_user (old_ka.sa.sa_restorer, &oact->sa_restorer)
            || __put_user (old_ka.sa.sa_flags, &oact->sa_flags) || __put_user (old_ka.sa.sa_mask.sig[0], &oact->sa_mask))
            return -EFAULT;
    }
...
COMPAT_SYSCALL_DEFINE3 (sigaction, int, sig, const struct compat_old_sigaction __user*, act, struct compat_old_sigaction __user*, oact)
{
    struct k_sigaction  new_ka, old_ka;
    int                 ret;
    ...
    if (act) {
        if (!access_ok (act, sizeof (*act)) || __get_user (handler, &act->sa_handler) || __get_user (restorer, &act->sa_restorer)
            || __get_user (new_ka.sa.sa_flags, &act->sa_flags) || __get_user (mask, &act->sa_mask))
            return -EFAULT;
...
#    endif
    ...
    if (!ret && oact) {
        if (!access_ok (oact, sizeof (*oact)) || __put_user (ptr_to_compat (old_ka.sa.sa_handler), &oact->sa_handler)
            || __put_user (ptr_to_compat (old_ka.sa.sa_restorer), &oact->sa_restorer) || __put_user (old_ka.sa.sa_flags, &oact->sa_flags)
...
#    ifdef CONFIG_SYSCTL_EXCEPTION_TRACE
        {.procname = "exception-trace", .data = &show_unhandled_signals, .maxlen = sizeof (int), .mode = 0644, .proc_handler = proc_dointvec},
#    endif
};
