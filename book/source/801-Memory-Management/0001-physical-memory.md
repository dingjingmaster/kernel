...
初始化 init_mm，设置代码段的起始、结束位置，设置数据段的起始、结束位置，设置堆内存起始、结束位置(`setup_initial_init_mm`)：
```c
...
检测CPU是否支持 NX 功能(禁止数据段执行)`x86_configure_nx`：
...
加入是 EFI（Extensible Firmware Interface）环境下，将特定内存范围标记为预留（reserved），确保这些区域不会被后续的内存分配操作使用`efi_memblock_x86_reserve_range`(使用`memblock_reserve`)。
