...
在用户态调用系统调用接口时，用户程序必须按照事先约定的寄存器传递参数，一般约定如下：
- RAX存放系统调用号
- 参数传递依次使用：RDI、RSI、RDX、R10、R8、R9寄存器最多传递6个参数
- 返回值放到RAX寄存器中

### syscall指令执行
当所有参数准备好之后，用户程序执行`syscall`指令。这条指令是cpu提供的专门用于用户态到内核态切换的机制。syscall指令的执行涉及以下几个关键点：
- 快速切换：syscall通过硬件支持直接切换到内核态，大大减少了切换开销
- 寄存器保存：执行syscall指令时候，CPU会自动保存部分寄存器信息：
    - RCX：保存下一条指令的地址(即用户态调用syscall后继续执行的位置)
    - R11：保存原始的RFLAGS寄存器的值，这样在返回用户态时候可以恢复中断状态等标志
- 特权级切换：syscall会使CPU从用户态(特权级3)切换到内核态(特权级0)

### MSRs管理系统调用
CPU内部有一组专用的Model Specific Registers(MSRs)用于管理系统调用的行为，其中最关键的有：
- LSTAR(Long System Target Address Register)：存储了内核中系统调用入口函数的地址。当执行syscall时候，CPU会跳转到LSTAR指定的地址。
- STAR：用于设置系统调用和sysret指令切换时候使用的段寄存器(如：CS、SS)的选择值
- SFMASK：指定在进入内核态时候需要屏蔽的RFLAGS位(例如中断标志位)。

在系统初始化过程中(通常在内核启动时候)，内核会写入MSR寄存器，为syscall指令的正常工作做准备。

### 进入内核态后处理流程

进入到内核态之后处理流程大致如下：
1. 跳转到系统调用入口：根据MSR中的LSTAR值，CPU跳转到内核定义的系统调用入口。此入口通常是一个汇编语言编写的入口例程，它完成初步的现场保存、寄存器设置和参数恢复等工作
2. 系统调用号分发：在入口例程中，内核会从RAX中取出系统调用号，并利用这个号在内核的系统调用表(sys_call_table)中查找相应的系统调用处理函数
3. 执行系统调用函数：系统调用处理函数根据传入的参数进行操作(例如：文件操作、进程控制等)，执行完毕后，将返回值写入RAX，同时更新必要的内核状态
4. 返回用户态：系统调用处理完毕后，内核需要将处理结果返回到用户态。返回过程中：
    - 通过sysret指令(或在某些架构上可能使用其他返回指令)将CPU从内核态切换回用户态
    - sysret指令会恢复先前保存在RCX和R11中的用户指令地址和RFLAGS状态，从而使用户程序能从正确的位置继续执行，并获得系统调用的返回值(在RAX中)。

## 软中断实现系统调用

早期linux内核(Linux 2.4之前)系统调用基本上是通过`int 0x80`软中断来实现的。这种方式简单直接，但是由于需要保存和恢复较多的寄存器状态，其性能开销相对较高。

具体实现过程如下：
1. 参数准备：用户程序在调用系统调用前，会把系统调用号放到EAX寄存器中。系统调用所需的参数依次放入：EBX、ECX、EDX、ESI、EDI、EBP中。
2. 触发软中断：用户程序执行`int 0x80`指令，这一指令请求CPU从用户态切换到内核态
3. 进入内核态：执行`int 0x80`之后，CPU会自动保存当前的部分寄存器状态，并切换到内核态。随后CPU根据中断向量`0x80`查找到内核中对应的中断服务例程，也就是系统调用入口程序。
4. 系统调用调度：内核入口例程从EAX中读取系统调用号，并在系统调用表中查找对应的系统调用处理函数。内核调用该函数，传递从寄存器中获得的参数，执行所请求的操作(如文件操作、进程管理等)。
5. 返回用户态：系统调用函数执行完毕后，其返回值通常放回EAX寄存器。内核通过执行`iret`(或类似指令)恢复先前保存的用户态上下文，切换回用户态，并将结果返回给用户程序。


> 注意：Linux2.6之后版本，INtel引入了sysenter(以及对应的sysexit)指令，随后AMD和后来的x86_64架构推出了syscall指令，这些都是硬件层面的指令，用于更快速的在用户态和内核态之间切换：
> - 在32位Linux内核中，如果CPU支持sysenter，内核优先采用它来实现系统调用，以降低系统调用的开销
> - 在64位Linux内核(x86_64)中，系统调用则主要采用syscall指令，这一机制提供了更高效的调用方式。
