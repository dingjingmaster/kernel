...
您可以使用环境变量`GNUTARGET`、`LDEMULATION`和`COLLECT_NO_DEMANGLE`来改变ld的行为。

如果不使用‘-b’（或其同义词‘--format’），则`GNUTARGET`决定输入文件对象格式。该值应为一种输入格式的BFD名称之一。如果环境中没有`GNUTARGET`，ld将使用目标的自然格式。如果`GNUTARGET`设置为`default`，则BFD通过检查二进制输入文件来发现输入格式；这种方法通常会成功，但是存在潜在的模糊性，因为没有方法可以确保用于指定对象文件格式的幻数是唯一的。但是，在每个系统上配置BFD的过程中，都会将该系统的常规格式放在查找列表的首位，从而消除了歧义。

如果不使用‘-m’选项，则`LDEMULATION`决定默认模拟。模拟可以影响链接器行为的各个方面，特别是默认链接器脚本。您可以使用‘--verbose’或‘-V ’选项列出可用的模拟。如果未使用‘-m’选项，并且未定义`LDEMULATION`环境变量，则默认仿真取决于如何配置链接器。

通常情况下，链接器将默认为搜索符号。但是，如果在环境中设置了`COLLECT_NO_DEMANGLE`，那么它将默认不要求符号。gcc链接器包装程序以类似的方式使用该环境变量。默认值可以被‘--demangle’和‘--no-demangle’选项覆盖。

如果PE/COFF特定的`--insert-timestamp`被设置，并且定义了`SOURCE_DATE_EPOCH`环境变量，那么这个变量中的时间戳值将被插入到COFF报头中，而不是当前时间。
