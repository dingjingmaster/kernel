...
int check_cpu (int* cpu_level_ptr, int* req_level_ptr, u32** err_flags_ptr)
{
    ...
    if (err == 0x01 && !(err_flags[0] & ~((1 << X86_FEATURE_XMM) | (1 << X86_FEATURE_XMM2))) && is_amd ()) {
        ...
        err = check_cpuflags ();
    } else if (err == 0x01 && !(err_flags[0] & ~(1 << X86_FEATURE_CX8)) && is_centaur () && cpu.model >= 6) {
...
int check_knl_erratum (void)
{
	 ...
	 */
    if (!is_intel () || cpu.family != 6 || cpu.model != 0x57 /*INTEL_XEON_PHI_KNL*/)
        return 0;
