...
void recalibrate_cpu_khz (void)
{
    ...
    cpu_data (0).loops_per_jiffy =
            cpufreq_scale (cpu_data (0).loops_per_jiffy, cpu_khz_old, cpu_khz);
#endif
}
...
static int time_cpufreq_notifier (struct notifier_block* nb, unsigned long val, void* data)
{
    ...
    if ((val == CPUFREQ_PRECHANGE && freq->old < freq->new) ||
        ...
        boot_cpu_data.loops_per_jiffy = cpufreq_scale (loops_per_jiffy_ref, ref_freq, freq->new);
...
static void __init detect_art (void)
{
     ...
     */
    if (boot_cpu_has (X86_FEATURE_HYPERVISOR) || !boot_cpu_has (X86_FEATURE_NONSTOP_TSC) ||
        !boot_cpu_has (X86_FEATURE_TSC_ADJUST) || tsc_async_resets)
        return;

    cpuid (ART_CPUID_LEAF, &art_base_clk.denominator, &art_base_clk.numerator,
           &art_base_clk.freq_khz, &unused);
...
{
    return !(clocksource_tsc.flags & CLOCK_SOURCE_MUST_VERIFY) && tsc_as_watchdog &&
           !no_tsc_watchdog;
}
...
#    define RTSC_SUSP 0x100
     ...
     */
    if (boot_cpu_has (X86_FEATURE_CONSTANT_TSC) && boot_cpu_has (X86_FEATURE_NONSTOP_TSC) &&
        boot_cpu_has (X86_FEATURE_TSC_ADJUST) && topology_max_packages () <= 4)
...
restart:
    ...
    if (boot_cpu_has (X86_FEATURE_TSC_KNOWN_FREQ)) {
        ...
        if (abs (tsc_khz - freq) > (tsc_khz >> 11)) {
            pr_warn ("Warning: TSC freq calibrated by CPUID/MSR differs from what is calibrated by "
                     "HW timer, please check with vendor!!\n");
            pr_info ("Previous calibrated TSC freq:\t %lu.%03lu MHz\n",
                     (unsigned long)tsc_khz / 1000, (unsigned long)tsc_khz % 1000);
        }

        pr_info ("TSC freq recalibrated by [%s]:\t %lu.%03lu MHz\n", hpet ? "HPET" : "PM_TIMER",
                 (unsigned long)freq / 1000, (unsigned long)freq % 1000);
...
static bool __init determine_cpu_tsc_frequencies (bool early)
{
    ...
    if (cpu_khz != tsc_khz) {
        pr_info ("Detected %lu.%03lu MHz TSC", (unsigned long)tsc_khz / KHZ,
