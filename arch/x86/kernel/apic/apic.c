...
{
    return (old & APIC_EILVT_MASKED) || (new == APIC_EILVT_MASKED) ||
...
static struct clock_event_device lapic_clockevent = {
        .name     = "lapic",
        .features = CLOCK_EVT_FEAT_PERIODIC | CLOCK_EVT_FEAT_ONESHOT | CLOCK_EVT_FEAT_C3STOP |
...
static int __init calibrate_by_pmtimer (u32 deltapm, long* delta, long* deltatsc)
{
...
static int __init lapic_init_clockevent (void)
{
    ...
    lapic_clockevent.mult =
            div_sc (lapic_timer_period / APIC_DIVISOR, TICK_NSEC, lapic_clockevent.shift);
    lapic_clockevent.max_delta_ns    = clockevent_delta2ns (0x7FFFFFFF, &lapic_clockevent);
...
bool __init apic_needs_pit (void)
{
     ...
     */
    if (apic_intr_mode == APIC_PIC || apic_intr_mode == APIC_VIRTUAL_WIRE_NO_CONFIG)
        return true;
...
static int __init                     __apic_intr_mode_select (void)
{
    ...
    if (!boot_cpu_has (X86_FEATURE_APIC) && APIC_INTEGRATED (boot_cpu_apic_version)) {
        apic_is_disabled = true;
...
void __init        apic_intr_mode_init (void)
{
    ...
    case APIC_VIRTUAL_WIRE: pr_info ("APIC: Switch to virtual wire mode setup\n"); break;
...
static void setup_local_APIC (void)
{
     ...
     */
    if ((!cpu && apic_extnmi != APIC_EXTNMI_NONE) || apic_extnmi == APIC_EXTNMI_ALL)
        value = APIC_DM_NMI;
    else
...
/**
 * 检测系统是否支持或正确配置了 x2APIC（Extended x86 Processor Interrupt Control）硬件功能，并验证其运行状态以避免因兼容性问题导致系统不稳定
 */
void __init check_x2apic (void)
{
...
static bool __init detect_init_APIC (void)
{
    ...
    case X86_VENDOR_AMD:
        if ((boot_cpu_data.x86 == 6 && boot_cpu_data.x86_model > 1) || (boot_cpu_data.x86 >= 15))
            break;
        goto no_apic;
    case X86_VENDOR_HYGON: break;
    case X86_VENDOR_INTEL:
        if (boot_cpu_data.x86 == 6 || boot_cpu_data.x86 == 15 ||
            (boot_cpu_data.x86 == 5 && boot_cpu_has (X86_FEATURE_APIC)))
            break;
        goto no_apic;
    default: goto no_apic;
    }
...
void disconnect_bsp_APIC (int virt_wire_setup)
{
    ...
    if (!virt_wire_setup) {
        ...
        value &= ~(APIC_MODE_MASK | APIC_SEND_PENDING | APIC_INPUT_POLARITY | APIC_LVT_REMOTE_IRR |
