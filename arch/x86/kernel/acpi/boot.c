 ...
 */
static u32 isa_irq_to_gsi[NR_IRQS_LEGACY] __read_mostly = {0, 1, 2,  3,  4,  5,  6,  7,
...
static int __init acpi_parse_madt (struct acpi_table_header* table)
{
    ...
    if (acpi_gbl_FADT.header.revision > 6 ||
        (acpi_gbl_FADT.header.revision == 6 && acpi_gbl_FADT.minor_revision >= 3))
...
static void mp_config_acpi_gsi (struct device* dev, u32 gsi, int trigger, int polarity)
{
...
void __init acpi_pic_sci_set_trigger (unsigned int irq, u16 trigger)
{
     ...
     */
    switch (trigger) {
    case 1: /* Edge - clear */ new &= ~mask; break;
    case 3: /* Level - set */ new |= mask; break;
    }
...
{
    if (isa_irq < nr_legacy_irqs () && isa_irq_to_gsi[isa_irq] != INVALID_ACPI_IRQ) {
...
#ifdef CONFIG_X86_LOCAL_APIC
static int
acpi_register_gsi_ioapic (struct device* dev, u32 gsi, int trigger, int polarity)
{
    int irq = gsi;
#    ifdef CONFIG_X86_IO_APIC
    int                   node;
...
int (*__acpi_register_gsi) (struct device* dev,
                            u32            gsi,
                            int            trigger,
                            int            polarity) = acpi_register_gsi_pic;
...
#    ifdef CONFIG_X86_64
    ...
    if (hpet_address == 0xfed0000000000000UL) {
        if (!hpet_force_user) {
            pr_warn ("HPET id: %#x base: 0xfed0000000000000 is bogus, try hpet=force on "
                     "the kernel command line to fix it up to 0xfed00000.\n",
                     hpet_tbl->id);
            hpet_address = 0;
            return 0;
        }
...
static int __init acpi_parse_madt_lapic_entries (void)
{
    ...
    count = acpi_table_parse_madt (ACPI_MADT_TYPE_LOCAL_SAPIC, acpi_parse_sapic,
                                   MAX_LOCAL_APIC);

    if (!count) {
        count   = acpi_table_parse_madt (ACPI_MADT_TYPE_LOCAL_APIC, acpi_parse_lapic,
                                         MAX_LOCAL_APIC);
        x2count = acpi_table_parse_madt (ACPI_MADT_TYPE_LOCAL_X2APIC, acpi_parse_x2apic,
                                         MAX_LOCAL_APIC);
    }
    if (!count && !x2count) {
    ...
    count   = acpi_table_parse_madt (ACPI_MADT_TYPE_LOCAL_APIC_NMI, acpi_parse_lapic_nmi,
                                     0);
...
static int __init acpi_parse_madt_ioapic_entries (void)
{
    ...
    count = acpi_table_parse_madt (ACPI_MADT_TYPE_NMI_SOURCE, acpi_parse_nmi_src,
                                   nr_irqs);
    if (count < 0) {
...
#    ifdef CONFIG_ACPI_MADT_WAKEUP
             ...
             */
            acpi_table_parse_madt (ACPI_MADT_TYPE_MULTIPROC_WAKEUP, acpi_parse_mp_wake,
                                   1);
#    endif
        }
        if (error == -EINVAL) {
     ...
     */
        {
                ...
                .ident    = "ASUS PR-DLS",
                .matches  = {DMI_MATCH (DMI_BOARD_VENDOR, "ASUSTeK Computer INC."),
...
int __init acpi_mps_check (void)
{
    ...
    if (acpi_disabled || acpi_noirq) {
        pr_warn ("MPS support code is not built-in, using acpi=off or acpi=noirq or "
                 "pci=noacpi may have problem\n");
        return 1;
    }
#endif
    return 0;
...
void __iomem* (*acpi_os_ioremap) (acpi_physical_address phys,
                                  acpi_size             size) = x86_acpi_os_ioremap;
