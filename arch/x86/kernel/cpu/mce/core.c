...
struct mca_config mca_cfg         __read_mostly = {.bootlog = -1, .monarch_timeout = -1};
 ...
 */
DEFINE_PER_CPU (mce_banks_t, mce_poll_banks) = {[0 ... BITS_TO_LONGS (MAX_NR_BANKS) - 1] = ~0UL};
...
static void __print_mce (struct mce* m)
{
    pr_emerg (HW_ERR "CPU %d: Machine Check%s: %Lx Bank %d: %016Lx\n", m->extcpu,
              (m->mcgstatus & MCG_STATUS_MCIP ? " Exception" : ""), m->mcgstatus, m->bank,
              m->status);
    ...
    pr_emerg (HW_ERR "PROCESSOR %u:%x TIME %llu SOCKET %u APIC %x microcode %x\n", m->cpuvendor,
              m->cpuid, m->time, m->socketid, m->apicid, m->microcode);
}
...
{
    if (wrmsr) {
        pr_emerg ("MSR access error: WRMSR to 0x%x (tried to write 0x%08x%08x) at rIP: 0x%lx "
                  "(%pS)\n",
                  (unsigned int)regs->cx, (unsigned int)regs->dx, (unsigned int)regs->ax, regs->ip,
                  (void*)regs->ip);
    } else {
...
bool mce_usable_address (struct mce* m)
{
    ...
    switch (m->cpuvendor) {
    case X86_VENDOR_AMD: return amd_mce_usable_address (m);

    case X86_VENDOR_INTEL:
    case X86_VENDOR_ZHAOXIN: return intel_mce_usable_address (m);
...
bool mce_is_memory_error (struct mce* m)
{
    switch (m->cpuvendor) {
    case X86_VENDOR_AMD:
    case X86_VENDOR_HYGON: return amd_mce_is_memory_error (m);
    ...
    case X86_VENDOR_ZHAOXIN:
		 ...
		 */
        return (m->status & 0xef80) == BIT (7) || (m->status & 0xef00) == BIT (8) ||
               (m->status & 0xeffc) == 0xc;
...
static int mce_early_notifier (struct notifier_block* nb, unsigned long val, void* data)
{
...
static int uc_decode_notifier (struct notifier_block* nb, unsigned long val, void* data)
{
    ...
    if (mce->severity != MCE_AO_SEVERITY && mce->severity != MCE_DEFERRED_SEVERITY)
        return NOTIFY_DONE;
 ...
 */
static __always_inline void quirk_sandybridge_ifu (int bank, struct mce* m, struct pt_regs* regs)
{
    if (bank != 0)
        return;
    if ((m->mcgstatus & (MCG_STATUS_EIPV | MCG_STATUS_RIPV)) != 0)
        return;
    if ((m->status & (MCI_STATUS_OVER | MCI_STATUS_UC | MCI_STATUS_EN | MCI_STATUS_MISCV |
                      MCI_STATUS_ADDRV | MCI_STATUS_PCC | MCI_STATUS_S | MCI_STATUS_AR | MCACOD)) !=
        (MCI_STATUS_UC | MCI_STATUS_EN | MCI_STATUS_MISCV | MCI_STATUS_ADDRV | MCI_STATUS_S |
...
static noinstr bool quirk_skylake_repmov (void)
{
	 ...
	 * sync is needed.
	 */
    if (!(mcgstatus & MCG_STATUS_LMCES) || !(misc_enable & MSR_IA32_MISC_ENABLE_FAST_STRING))
        return false;
    ...
    if ((mc1_status &
         (MCI_STATUS_VAL | MCI_STATUS_OVER | MCI_STATUS_UC | MCI_STATUS_EN | MCI_STATUS_ADDRV |
          MCI_STATUS_MISCV | MCI_STATUS_PCC | MCI_STATUS_AR | MCI_STATUS_S)) ==
        (MCI_STATUS_VAL | MCI_STATUS_UC | MCI_STATUS_EN | MCI_STATUS_ADDRV | MCI_STATUS_MISCV |
...
mce_no_way_out (struct mce* m, char** msg, unsigned long* validp, struct pt_regs* regs)
{
    char* tmp = *msg;
    int   i;
...
static noinstr int mce_start (int* no_way_out)
{
    ...
    while (raw_atomic_read (&mce_callin) != num_online_cpus ()) {
        if (mce_timed_out (&timeout, "Timeout: Not all CPUs entered broadcast exception handler")) {
    ...
    if (order == 1) {
        ...
        while (raw_atomic_read (&mce_executing) < order) {
            if (mce_timed_out (&timeout, "Timeout: Subject CPUs unable to finish machine check "
                                         "processing")) {
                raw_atomic_set (&global_nwo, 0);
                goto out;
            }
...
static noinstr int mce_end (int order)
{
    ...
    if (order == 1) {
        ...
        while (atomic_read (&mce_executing) <= num_online_cpus ()) {
            if (mce_timed_out (&timeout, "Timeout: Monarch CPU unable to finish machine check "
                                         "processing"))
                goto reset;
            ndelay (SPINUNIT);
        }
        ...
        while (atomic_read (&mce_executing) != 0) {
            if (mce_timed_out (&timeout, "Timeout: Monarch CPU did not finish machine check "
                                         "processing"))
                goto reset;
            ndelay (SPINUNIT);
        }
...
static noinstr bool mce_check_crashing_cpu (void)
{
    ...
    if (arch_cpu_is_offline (cpu) || (crashing_cpu != -1 && crashing_cpu != cpu)) {
        u64 mcgstatus;
...
static __always_inline int __mc_scan_banks (struct mce*     m,
                                            struct pt_regs* regs,
                                            struct mce*     final,
                                            ...
                                            int*            worst)
{
    ...
    for (i = 0; i < this_cpu_read (mce_num_banks); i++) {
		 ...
		 */
        if (!(m->status & (cfg->ser ? MCI_STATUS_S : MCI_STATUS_UC)) && !no_way_out)
            continue;
		 ...
		 */
        if ((severity == MCE_KEEP_SEVERITY || severity == MCE_UCNA_SEVERITY) && !no_way_out)
            continue;
...
static int __mcheck_cpu_apply_quirks (struct cpuinfo_x86* c)
{
    ...
    if (c->x86_vendor == X86_VENDOR_INTEL) {
		 ...
		 */
        if ((c->x86 > 6 || (c->x86 == 6 && c->x86_model >= 0xe)) && cfg->monarch_timeout < 0)
...
static void mce_centaur_feature_init (struct cpuinfo_x86* c)
{
    ...
    /*
	  ...
	  * synchronization with a one second timeout.
	  */
    if ((c->x86 == 6 && c->x86_model == 0xf && c->x86_stepping >= 0xe) || c->x86 > 6) {
        if (cfg->monarch_timeout < 0)
            cfg->monarch_timeout = USEC_PER_SEC;
    }
}
...
static void mce_zhaoxin_feature_init (struct cpuinfo_x86* c)
{
	 ...
	 */
    if ((c->x86 == 7 && c->x86_model == 0x1b) || (c->x86_model == 0x19 || c->x86_model == 0x1f)) {
...
{
    switch (c->x86_vendor) {
    case X86_VENDOR_INTEL: mce_intel_feature_init (c); break;
    ...
    case X86_VENDOR_HYGON: mce_hygon_feature_init (c); break;

    case X86_VENDOR_CENTAUR: mce_centaur_feature_init (c); break;

    case X86_VENDOR_ZHAOXIN: mce_zhaoxin_feature_init (c); break;
...
{
    switch (c->x86_vendor) {
    case X86_VENDOR_INTEL: mce_intel_feature_clear (c); break;

    case X86_VENDOR_ZHAOXIN: mce_zhaoxin_feature_clear (c); break;
...
static ssize_t
set_bank (struct device* s, struct device_attribute* attr, const char* buf, size_t size)
{
    u8               bank = attr_to_bank (attr)->bank;
    struct mce_bank* b;
    u64 new;
...
static ssize_t
set_ignore_ce (struct device* s, struct device_attribute* attr, const char* buf, size_t size)
{
    u64 new;
...
static ssize_t
set_cmci_disabled (struct device* s, struct device_attribute* attr, const char* buf, size_t size)
{
    u64 new;
...
static struct dev_ext_attribute dev_attr_check_interval = {
        __ATTR (check_interval, 0644, device_show_int, store_int_with_restart), &check_interval};

static struct dev_ext_attribute dev_attr_ignore_ce = {
        __ATTR (ignore_ce, 0644, device_show_bool, set_ignore_ce), &mca_cfg.ignore_ce};

static struct dev_ext_attribute dev_attr_cmci_disabled = {
        __ATTR (cmci_disabled, 0644, device_show_bool, set_cmci_disabled), &mca_cfg.cmci_disabled};

static struct device_attribute* mce_device_attrs[] = {&dev_attr_check_interval.attr,
...
static __init int mcheck_init_device (void)
{
    ...
    err = cpuhp_setup_state (CPUHP_X86_MCE_DEAD, "x86/mce:dead", NULL, mce_cpu_dead);
    if (err)
        goto err_out_mem;
	 ...
	 */
    err = cpuhp_setup_state (CPUHP_AP_ONLINE_DYN, "x86/mce:online", mce_cpu_online,
                             mce_cpu_pre_down);
    if (err < 0)
        goto err_out_online;
...
static void __init mcheck_debugfs_init (void)
{
    ...
    debugfs_create_file_unsafe ("fake_panic", 0444, dmce, NULL, &fake_panic_fops);
}
#else
static void __init mcheck_debugfs_init (void)
{
