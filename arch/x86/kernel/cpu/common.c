...
static struct ppin_info
{
    ...
    int msr_ppin;
        ...
        [X86_VENDOR_INTEL] = {.feature = X86_FEATURE_INTEL_PPIN, .msr_ppin_ctl = MSR_PPIN_CTL, .msr_ppin = MSR_PPIN},
        [X86_VENDOR_AMD]   = {.feature = X86_FEATURE_AMD_PPIN, .msr_ppin_ctl = MSR_AMD_PPIN_CTL, .msr_ppin = MSR_AMD_PPIN},
};

static const struct x86_cpu_id ppin_cpuids[] = {X86_MATCH_FEATURE (X86_FEATURE_AMD_PPIN, &ppin_info[X86_VENDOR_AMD]),
...
static const unsigned long cr4_pinned_mask = X86_CR4_SMEP | X86_CR4_SMAP | X86_CR4_UMIP | X86_CR4_FSGSBASE | X86_CR4_CET | X86_CR4_FRED;
...
static __always_inline void setup_cet (struct cpuinfo_x86* c)
{
    ...
    user_shstk = cpu_feature_enabled (X86_FEATURE_SHSTK) && IS_ENABLED (CONFIG_X86_USER_SHADOW_STACK);
...
{
    if (!(cpu_feature_enabled (X86_FEATURE_IBT) || cpu_feature_enabled (X86_FEATURE_SHSTK)))
        return;
...
void get_cpu_address_sizes (struct cpuinfo_x86* c)
{
    ...
    if (!cpu_has (c, X86_FEATURE_CPUID) || (c->extended_cpuid_level < 0x80000008)) {
...
#define VULNBL_INTEL_STEPPINGS(vfm, steppings, issues) X86_MATCH_VFM_STEPPINGS (vfm, steppings, issues)
...
{
    return (x86_arch_cap_msr & ARCH_CAP_FBSDP_NO && x86_arch_cap_msr & ARCH_CAP_PSDP_NO && x86_arch_cap_msr & ARCH_CAP_SBDR_SSDP_NO);
}
...
static void __init cpu_set_bug_bits (struct cpuinfo_x86* c)
{
    ...
    if (!cpu_matches (cpu_vuln_whitelist, NO_ITLB_MULTIHIT) && !(x86_arch_cap_msr & ARCH_CAP_PSCHANGE_MC_NO))
    ...
    if (!cpu_matches (cpu_vuln_whitelist, NO_SSB) && !(x86_arch_cap_msr & ARCH_CAP_SSB_NO) && !cpu_has (c, X86_FEATURE_AMD_SSB_NO))
     ...
     */
    if ((x86_arch_cap_msr & ARCH_CAP_IBRS_ALL) || (cpu_has (c, X86_FEATURE_AUTOIBRS) && !cpu_feature_enabled (X86_FEATURE_SEV_SNP))) {
        setup_force_cpu_cap (X86_FEATURE_IBRS_ENHANCED);
        if (!cpu_matches (cpu_vuln_whitelist, NO_EIBRS_PBRSB) && !(x86_arch_cap_msr & ARCH_CAP_PBRSB_NO))
            setup_force_cpu_bug (X86_BUG_EIBRS_PBRSB);
    }

    if (!cpu_matches (cpu_vuln_whitelist, NO_MDS) && !(x86_arch_cap_msr & ARCH_CAP_MDS_NO)) {
     ...
     */
    if (!(x86_arch_cap_msr & ARCH_CAP_TAA_NO) && (cpu_has (c, X86_FEATURE_RTM) || (x86_arch_cap_msr & ARCH_CAP_TSX_CTRL_MSR)))
     ...
     * SRBDS.
     */
    if ((cpu_has (c, X86_FEATURE_RDRAND) || cpu_has (c, X86_FEATURE_RDSEED)) && cpu_matches (cpu_vuln_blacklist, SRBDS | MMIO_SBDS))
     ...
     */
    if (cpu_matches (cpu_vuln_blacklist, GDS) && !(x86_arch_cap_msr & ARCH_CAP_GDS_NO) && boot_cpu_has (X86_FEATURE_AVX))
    ...
    if (!(x86_arch_cap_msr & ARCH_CAP_BHI_NO) && !cpu_matches (cpu_vuln_whitelist, NO_BHI)
        && (boot_cpu_has (X86_FEATURE_IBRS_ENHANCED) || boot_cpu_has (X86_FEATURE_HYPERVISOR)))
 ...
 *
 * detect_nopl 函数的作用是检测 CPU 是否支持 NOPL（No Operation with Lock Prefix）指令，
 * 从而决定是否启用基于此指令的安全机制（如防止敏感代码段被执行）。
 *
 * NOPL 指令是一条空操作指令，但其关键在于锁前缀。
 * CPU 支持 LOCK 前缀时，LOCK NOOP 会被视为对内存的原子操作，但实际不执行任何操作。
 */
...
static inline void idt_syscall_init (void)
{
     ...
     */
    wrmsrl (MSR_SYSCALL_MASK, X86_EFLAGS_CF | X86_EFLAGS_PF | X86_EFLAGS_AF | X86_EFLAGS_ZF | X86_EFLAGS_SF | X86_EFLAGS_TF | X86_EFLAGS_IF | X86_EFLAGS_DF
                                      | X86_EFLAGS_OF | X86_EFLAGS_IOPL | X86_EFLAGS_NT | X86_EFLAGS_RF | X86_EFLAGS_AC | X86_EFLAGS_ID);
}
...
void cpu_init (void)
{
    ...
    if (IS_ENABLED (CONFIG_X86_64) || cpu_feature_enabled (X86_FEATURE_VME) || boot_cpu_has (X86_FEATURE_TSC) || boot_cpu_has (X86_FEATURE_DE))
...
void store_cpu_caps (struct cpuinfo_x86* curr_info)
{
    ...
    memcpy (&curr_info->x86_capability, &boot_cpu_data.x86_capability, sizeof (curr_info->x86_capability));
...
void microcode_check (struct cpuinfo_x86* prev_info)
{
    ...
    if (!memcmp (&prev_info->x86_capability, &curr_info.x86_capability, sizeof (prev_info->x86_capability)))
        return;
