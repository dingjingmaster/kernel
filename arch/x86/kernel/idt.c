 ...
 */
#    define ISTG(_vector, _addr, _ist) G (_vector, _addr, _ist + 1, GATE_INTERRUPT, DPL0, __KERNEL_CS)
#else
 ...
 *
 * On x86_64 these traps do not use interrupt stacks as they can't work
 * before cpu_init() is invoked and sets up TSS. The IST variants are
 * installed after that.
 *
 * 用于在内核启动的早期阶段设置中断描述符表（IDT, Interrupt Descriptor Table）中的早期陷阱（traps）。
 * 这些陷阱通常用于处理在系统初始化过程中可能发生的中断或异常，例如除零错误、调试异常等。
 */
