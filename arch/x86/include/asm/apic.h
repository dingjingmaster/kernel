...
static inline void native_apic_mem_write (u32 reg, u32 v)
{
    ...
    alternative_io ("movl %0, %1", "xchgl %0, %1", X86_BUG_11AP, ASM_OUTPUT2 ("=r"(v), "=m"(*addr)),
...
{
    if (reg == APIC_DFR || reg == APIC_ID || reg == APIC_LDR || reg == APIC_LVR)
        return;
...
struct apic
{
    ...
    u32 disable_esr : 1, dest_mode_logical : 1, x2apic_set_max_apicid : 1, nmi_to_offline_cpu : 1;
...
#define apic_driver(sym)                                                                   \
    static const struct apic* __apicdrivers_##sym __used __aligned (sizeof (struct apic*)) \
...
/**
 * APIC(Advanced Programmable Interrupt Controller)负责处理中断和时钟事件，是现代操作系统与硬件交互的关键部分。
 *
 * 内核中用于初始化和配置x86 架构的 APIC 的核心函数，确保操作系统能够正确处理中断、时钟事件和多核协同
 * 1. 核心功能：
 *    - 寄存器配置：设置 APIC 的基地址（APIC_BASE 寄存器）、中断向量表（IVT）映射、优先级（APIC_PRI 寄存器）等关键参数
 *    - 模式启用：启用 APIC 和本地 APIC（LAPIC），通过设置 APIC_CNTL 寄存器的 APIC_EN 位
 * 2. 中断向量表配置
*    - 内核中断处理函数绑定：将 Linux 内核定义的中断处理函数（如 do_IRQ）地址写入 APIC 的 IVT（Interrupt Vector Table）中
*    - 兼容性处理：对于旧版 BIOS 或不支持 APIC 的系统，回退到传统的 PIC（Programmable Interrupt Controller）模式
*  3. 多核SMP支持
*    - IPI（Inter-Processor Interrupt）广播：配置 APIC 支持跨核心中断广播，用于唤醒休眠的核心或触发所有核心执行特定操作（如同步时钟）
*    - 自旋锁初始化：为多核环境初始化自旋锁（如 spin_lock_irqsave），确保在 APIC 中断上下文中安全访问共享资源
 */
...
#    define apic_update_callback(_callback, _fn)                           \
        {                                                                  \
            ...
            pr_info ("APIC: %s() replaced with %ps()\n", #_callback, _fn); \
                ...
                #    define DECLARE_APIC_CALL(__cb) DECLARE_STATIC_CALL (apic_call_##__cb, *apic->__cb)
