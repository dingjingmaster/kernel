...
#define IO_BITMAP_OFFSET_VALID_MAP (offsetof (struct tss_struct, io_bitmap.bitmap) - offsetof (struct tss_struct, x86_tss))

#define IO_BITMAP_OFFSET_VALID_ALL (offsetof (struct tss_struct, io_bitmap.mapall) - offsetof (struct tss_struct, x86_tss))
 ...
 */
#    define __KERNEL_TSS_LIMIT (IO_BITMAP_OFFSET_VALID_ALL + IO_BITMAP_BYTES + sizeof (unsigned long) - 1)
#else
#    define __KERNEL_TSS_LIMIT (offsetof (struct tss_struct, x86_tss) + sizeof (struct x86_hw_tss) - 1)
#endif
...
{
    return (unsigned long)(current_top_of_stack () - current_stack_pointer) < THREAD_SIZE;
}
...
{
    alternative_input (BASE_PREFETCH, "prefetchnta %1", X86_FEATURE_XMM, "m"(*(const char*)x));
}
...
{
    alternative_input (BASE_PREFETCH, "prefetchw %1", X86_FEATURE_3DNOWPREFETCH, "m"(*(const char*)x));
}
