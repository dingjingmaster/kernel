...
#    define LOADED_MM_SWITCHING ((struct mm_struct*)1UL)
     ...
     *
     * 如果已设置，我们更改了页面表，因此需要对所有上下文（又称 PCID / ASID）进行无效处理。
     * 这就要求我们在下一次上下文切换时，对所有未加载的ctxs[]进行无效处理。
     *
     * 当前的 ctx 在运行过程中保持最新，无需进行无效处理。
     */
...
#    define flush_tlb_range(vma, start, end)                                                    \
        flush_tlb_mm_range ((vma)->vm_mm, start, end,                                           \
                            ((vma)->vm_flags & VM_HUGETLB) ? huge_page_shift (hstate_vma (vma)) \
                                                           : PAGE_SHIFT,                        \
                            false)
...
static inline bool
pte_flags_need_flush (unsigned long oldflags, unsigned long newflags, bool ignore_access)
{
    /*
     ...
     */
    const pteval_t flush_on_clear = _PAGE_DIRTY | _PAGE_PRESENT | _PAGE_ACCESSED;
    const pteval_t software_flags = _PAGE_SOFTW1 | _PAGE_SOFTW2 | _PAGE_SOFTW3 | _PAGE_SOFTW4 |
                                    _PAGE_SAVED_DIRTY;
    const pteval_t flush_on_change = _PAGE_RW | _PAGE_USER | _PAGE_PWT | _PAGE_PCD | _PAGE_PSE |
                                     _PAGE_GLOBAL | _PAGE_PAT | _PAGE_PAT_LARGE | _PAGE_PKEY_BIT0 |
                                     _PAGE_PKEY_BIT1 | _PAGE_PKEY_BIT2 | _PAGE_PKEY_BIT3 | _PAGE_NX;
...
static inline bool huge_pmd_needs_flush (pmd_t oldpmd, pmd_t newpmd)
{
     ...
     */
    return pte_flags_need_flush (pmd_flags (oldpmd), pmd_flags (newpmd), false);
}
