...
{
    const struct desc_ptr invalid_gdt = {.address = 0, .size = 0};
...
{
    const struct desc_ptr invalid_idt = {.address = 0, .size = 0};
...
#define LDT_empty(info)                                                                                                             \
    ((info)->base_addr == 0 && (info)->limit == 0 && (info)->contents == 0 && (info)->read_exec_only == 1 && (info)->seg_32bit == 0 \
     && (info)->limit_in_pages == 0 && (info)->seg_not_present == 1 && (info)->useable == 0)

/* Lots of programs expect an all-zero user_desc to mean "no segment at all". */
static inline bool LDT_zero (const struct user_desc* info)
{
    return (info->base_addr == 0 && info->limit == 0 && info->contents == 0 && info->read_exec_only == 0 && info->seg_32bit == 0 && info->limit_in_pages == 0
            && info->seg_not_present == 0 && info->useable == 0);
}
