...
static const unsigned long* const efi_tables[] = {
        &efi.acpi,           &efi.acpi20,        &efi.smbios,       &efi.smbios3, &uga_phys,
#ifdef CONFIG_X86_UV
        &uv_systab_phys,
#endif
        &efi_fw_vendor,      &efi_runtime,       &efi_config_table, &efi.esrt,    &prop_phys,
        &efi_mem_attr_table,
#ifdef CONFIG_EFI_RCI2_TABLE
        &rci2_table_phys,
#endif
        &efi.tpm_log,        &efi.tpm_final_log, &efi_rng_seed,
...
static void __init do_add_efi_memmap (void)
{
    ...
    for_each_efi_memory_desc (md) {
        ...
        case EFI_CONVENTIONAL_MEMORY:
            ...
            break;
        case EFI_ACPI_RECLAIM_MEMORY: e820_type = E820_TYPE_ACPI; break;
        case EFI_ACPI_MEMORY_NVS: e820_type = E820_TYPE_NVS; break;
        case EFI_UNUSABLE_MEMORY: e820_type = E820_TYPE_UNUSABLE; break;
        case EFI_PERSISTENT_MEMORY: e820_type = E820_TYPE_PMEM; break;
        default:
            /*
...
static bool __init efi_memmap_entry_valid (const efi_memory_desc_t* md, int i)
{
    ...
    if (end_hi) {
        pr_warn ("mem%02u: %s range=[0x%016llx-0x%llx%016llx] (invalid)\n", i,
                 efi_md_typeattr_format (buf, sizeof (buf), md), md->phys_addr, end_hi, end);
    } else {
        pr_warn ("mem%02u: %s range=[0x%016llx-0x%016llx] (invalid)\n", i,
                 efi_md_typeattr_format (buf, sizeof (buf), md), md->phys_addr, end);
    }
    return false;
}
    ...
    for_each_efi_memory_desc (md) {
        if (md->type == EFI_MEMORY_MAPPED_IO) {
            ...
            if (size >= 256 * 1024) {
                ...
                pr_info ("Not removing mem%02u: MMIO range=[0x%08llx-0x%08llx] (%lluKB) from e820 "
                         "map\n",
                         i, start, end, size >> 10);
            }
        }
...
static int __init efi_systab_init (unsigned long phys)
{
    ...
    if (efi_enabled (EFI_64BIT)) {
        ...
        if (efi_setup) {
            ...
            over4g |= data->fw_vendor > U32_MAX || data->tables > U32_MAX;
            ...
            over4g |= systab64->fw_vendor > U32_MAX || systab64->tables > U32_MAX;
        }
...
static int __init efi_config_init (const efi_config_table_type_t* arch_tables)
{
    ...
    ret = efi_config_parse_tables (config_tables, efi_nr_tables, arch_tables);
...
void __init efi_init (void)
{
    if (IS_ENABLED (CONFIG_X86_32) &&
        (boot_params.efi_info.efi_systab_hi || boot_params.efi_info.efi_memmap_hi)) {
...
static void __init efi_merge_regions (void)
{
    ...
    for_each_efi_memory_desc (md) {
        ...
        if (prev_md->type != md->type || prev_md->attribute != md->attribute) {
            prev_md = md;
            continue;
        }
...
static bool should_map_region (efi_memory_desc_t* md)
{
     ...
     */
    if (md->type == EFI_CONVENTIONAL_MEMORY && efi_soft_reserve_enabled () &&
     ...
     */
    if (efi_is_mixed ()) {
        if (md->type == EFI_CONVENTIONAL_MEMORY || md->type == EFI_LOADER_DATA ||
     ...
     */
    if (md->type == EFI_BOOT_SERVICES_CODE || md->type == EFI_BOOT_SERVICES_DATA)
        return true;
...
static void __init kexec_enter_virtual_mode (void)
{
    ...
    */
    ...
    if (efi_memmap_init_late (efi.memmap.phys_map, efi.memmap.desc_size * efi.memmap.nr_map)) {
...
static void __init __efi_enter_virtual_mode (void)
{
    ...
    status = efi_set_virtual_address_map (efi.memmap.desc_size * count, efi.memmap.desc_size,
                                          efi.memmap.desc_version, (efi_memory_desc_t*)pa,
                                          efi_systab_phys);
...
umode_t               efi_attr_is_visible (struct kobject* kobj, struct attribute* attr, int n)
{
    if (attr == &efi_attr_fw_vendor.attr) {
        if (efi_enabled (EFI_PARAVIRT) || efi_fw_vendor == EFI_INVALID_TABLE_ADDR)
            return 0;
