...
static void
choose_new_asid (struct mm_struct* next, u64 next_tlb_gen, u16* new_asid, bool* need_flush)
{
    u16 asid;
    ...
    for (asid = 0; asid < TLB_NR_DYN_ASIDS; asid++) {
        if (this_cpu_read (cpu_tlbstate.ctxs[asid].ctx_id) != next->context.ctx_id)
            continue;
...
static inline void invalidate_user_asid (u16 asid)
{
    ...
    __set_bit (kern_pcid (asid), (unsigned long*)this_cpu_ptr (&cpu_tlbstate.user_pcid_flush_mask));
}
...
static void
l1d_flush_evaluate (unsigned long prev_mm, unsigned long next_mm, struct task_struct* next)
{
...
static void cond_mitigation (struct task_struct* next)
{
    ...
    if (static_branch_likely (&switch_mm_cond_ibpb)) {
         ...
         */
        if (next_mm != prev_mm && (next_mm | prev_mm) & LAST_USER_MM_IBPB)
...
void switch_mm_irqs_off (struct mm_struct* unused, struct mm_struct* next, struct task_struct* tsk)
{
    ...
    if (WARN_ON_ONCE (__read_cr3 () !=
                      build_cr3 (prev->pgd, prev_asid, tlbstate_lam_cr3_mask ()))) {
        /*
    ...
    if (prev == next) {
         ...
         */
        if (WARN_ON_ONCE (prev != &init_mm && !cpumask_test_cpu (cpu, mm_cpumask (next))))
...
static void flush_tlb_func (void* info)
{
    ...
    if (unlikely (f->new_tlb_gen != TLB_GENERATION_INVALID && f->new_tlb_gen <= local_tlb_gen)) {
        /*
                         ...
                         bool              freed_tables)
{
    ...
    if ((end == TLB_FLUSH_ALL) || ((end - start) >> stride_shift) > tlb_single_page_flush_ceiling) {
        start = 0;
        end   = TLB_FLUSH_ALL;
    }
...
void flush_tlb_kernel_range (unsigned long start, unsigned long end)
{
    ...
    if (end == TLB_FLUSH_ALL || (end - start) > tlb_single_page_flush_ceiling << PAGE_SHIFT) {
        on_each_cpu (do_flush_tlb_all, NULL, 1);
    } else {
        struct flush_tlb_info* info;

        preempt_disable ();
        info = get_flush_tlb_info (NULL, start, end, 0, false, TLB_GENERATION_INVALID);
...
{
    count_vm_tlb_event (NR_TLB_LOCAL_FLUSH_ONE); // 什么也没干
     ...
     *
     * PTI(Page Table Isolation, 页表隔离)是 Linux 内核引入的一种安全机制，
     * 旨在防止CPU旁道攻击(如：meltdown 漏洞)影响内核数据的安全性。
     *
     * PTI主要在 x86_64 架构上启用，通过将用户态和内核态的页表完全隔离，防止用于态进程利用CPU的缓存投机执行
     * 来读取内核内存数据。
     *
     * 传统 Linux 内存管理中，内核地址空间始终映射在所有进程的页表中(只是在CR3控制位上设置不可访问)。
     * 恶意进程可以利用投机执行访问这些地址，借助缓存侧信道泄漏内核数据。
     *
     * 开启 PTI 之后，用户进程的页表不再包含内核页表，彻底杜绝了用户态访问任何内核地址的可能。
     * 用户态进程切换到内核态时候，才会加载完整的内核页表(通过CR3重新加载页表)。
     *
     *
     * 如果 PTI 关闭，那么 __flush_tlb_one_user() 就是 INVLPG 或其 paravirt 等效函数。
     * 即使使用 PCID，这也足够了：我们只有在内核映射使用全局 PTE 时才使用 PCID，
     * 而 INVLPG 会在所有地址空间刷新全局转换。
     *
     * 如果 PTI 开启，那么内核将使用非全局 PTE 映射，
     * __flush_tlb_one_user() 将为当前内核地址空间及其用户模式对应空间刷新给定地址，
     * 但不会为其他地址空间刷新地址。
     */
    flush_tlb_one_user (addr);
...
void arch_tlbbatch_flush (struct arch_tlbflush_unmap_batch* batch)
{
    ...
    info = get_flush_tlb_info (NULL, 0, TLB_FLUSH_ALL, 0, false, TLB_GENERATION_INVALID);
    /*
     * flush_tlb_multi() is not optimized for the common case in which only
     * a local TLB flush is needed. Optimize this use-case by calling
     * flush_tlb_func_local() directly in this case.
...
static ssize_t
tlbflush_read_file (struct file* file, char __user* user_buf, size_t count, loff_t* ppos)
{
    char         buf[32];
    unsigned int len;
...
static ssize_t
tlbflush_write_file (struct file* file, const char __user* user_buf, size_t count, loff_t* ppos)
{
    char    buf[32];
    ssize_t len;
    int     ceiling;
...
{
    debugfs_create_file ("tlb_single_page_flush_ceiling", S_IRUSR | S_IWUSR, arch_debugfs_dir, NULL,
                         &fops_tlbflush);
    return 0;
}
