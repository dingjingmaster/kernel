...
__ref void*                     alloc_low_pages (unsigned int num)
{
    ...
    if ((pgt_buf_end + num) > pgt_buf_top || !can_use_brk_pgt) {
        ...
        if (min_pfn_mapped < max_pfn_mapped) {
            ret = memblock_phys_alloc_range (PAGE_SIZE * num, PAGE_SIZE,
...
/**
 * Linux 内核在早期初始化阶段用于分配和管理页表缓冲区（Page Table Buffer）的核心功能，其作用可概括为以下方面：
 *  - 页表缓冲区分配: 在内核启动的早期阶段（通常在 setup_arch() 后），为内核的页表结构（如直接映射区、vmalloc 区域）分配连续的物理内存块，作为页表条目（PTE）的存储基础
 *  - 内存映射初始化: 使用分配的页表缓冲区初始化内核的直接映射区（Directly Mapped Region），将物理内存的起始部分（如 0x8000 到 0xC0000000）映射到内核虚拟地址空间
 *  - 支持高端内存: 当物理内存超过 MAXMEM_PFN（通常为 1GB）时，分配额外的页表缓冲区用于映射高端内存（vmemmap 区域），确保内核能访问全部物理内存
 */
 ...
 */
static const struct x86_cpu_id invlpg_miss_ids[] = {X86_MATCH_VFM (INTEL_ALDERLAKE, 0),
...
static int __meminit save_mr (struct map_range* mr,
                              int               nr_range,
...
static const char* page_size_string (struct map_range* mr)
{
     ...
     */
    if (IS_ENABLED (CONFIG_X86_32) && !IS_ENABLED (CONFIG_X86_PAE) &&
...
{
    nr_pfn_mapped =
            add_range_with_merge (pfn_mapped, E820_MAX_ENTRIES, nr_pfn_mapped, start_pfn, end_pfn);
...
{
    if (region_intersects (PFN_PHYS (pagenr), PAGE_SIZE, IORESOURCE_SYSTEM_RAM, IORES_DESC_NONE) !=
        REGION_DISJOINT) {
        /*
