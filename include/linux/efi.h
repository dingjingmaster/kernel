...
efi_get_wakeup_time_t (efi_bool_t* enabled, efi_bool_t* pending, efi_time_t* tm);
...
typedef efi_status_t
efi_get_next_variable_t (unsigned long* name_size, efi_char16_t* name, efi_guid_t* vendor);
typedef efi_status_t efi_set_variable_t (
        efi_char16_t* name, efi_guid_t* vendor, u32 attr, unsigned long data_size, void* data);
typedef efi_status_t efi_get_next_high_mono_count_t (u32* count);
typedef void         efi_reset_system_t (int           reset_type,
                                         efi_status_t  status,
...
typedef efi_status_t
efi_update_capsule_t (efi_capsule_header_t** capsules, unsigned long count, unsigned long sg_list);
typedef efi_status_t efi_query_capsule_caps_t (efi_capsule_header_t** capsules,
                                               unsigned long          count,
                                               u64*                   max_size,
                                               int*                   reset_type);
typedef efi_status_t
efi_query_variable_store_t (u32 attributes, unsigned long size, bool nonblocking);
...
#define UGA_IO_PROTOCOL_GUID \
    EFI_GUID (0x61a4d49e, 0x6f68, 0x4f1b, 0xb9, 0x22, 0xa8, 0x6e, 0xed, 0x0b, 0x07, 0xa2)
...
#define APPLE_PROPERTIES_PROTOCOL_GUID \
    EFI_GUID (0x91bd12fe, 0xf6c3, 0x44fb, 0xa5, 0xb7, 0x51, 0x22, 0xab, 0x30, 0x3a, 0xe0)
...
#define EFI_SMBIOS_PROTOCOL_GUID \
    EFI_GUID (0x03583ff6, 0xcb36, 0x4940, 0x94, 0x7e, 0xb9, 0xb3, 0x9f, 0x4a, 0xfa, 0xf7)
...
#define EFI_CC_BLOB_GUID \
    EFI_GUID (0x067b1f5f, 0xcf26, 0x44c5, 0x85, 0x54, 0x93, 0xd7, 0x77, 0x91, 0x2d, 0x42)
...
#define LINUX_EFI_SCREEN_INFO_TABLE_GUID \
    EFI_GUID (0xe03fc20a, 0x85dc, 0x406e, 0xb9, 0x0e, 0x4a, 0xb5, 0x02, 0x37, 0x1d, 0x95)
#define LINUX_EFI_ARM_CPU_STATE_TABLE_GUID \
    EFI_GUID (0xef79e4aa, 0x3c3d, 0x4989, 0xb9, 0x02, 0x07, 0xa9, 0x43, 0xe5, 0x50, 0xd2)
...
#define LINUX_EFI_TPM_EVENT_LOG_GUID \
    EFI_GUID (0xb7799cb0, 0xeca2, 0x4943, 0x96, 0x67, 0x1f, 0xae, 0x07, 0xb7, 0x47, 0xfa)
...
#define LINUX_EFI_UNACCEPTED_MEM_TABLE_GUID \
    EFI_GUID (0xd5d1de3c, 0x105c, 0x44f9, 0x9e, 0xa9, 0xbc, 0xef, 0x98, 0x12, 0x00, 0x31)
...
#define LINUX_EFI_LOADED_IMAGE_FIXED_GUID \
    EFI_GUID (0xf5a37b6d, 0x3344, 0x42a5, 0xb6, 0xbb, 0x97, 0x86, 0x48, 0xc1, 0x89, 0x0a)

/* OEM GUIDs */
#define DELLEMC_EFI_RCI2_TABLE_GUID \
    EFI_GUID (0x2d9f28a2, 0xa886, 0x456a, 0x97, 0xa8, 0xf1, 0x1e, 0xf2, 0x4f, 0xf4, 0x55)
...
#define OVMF_SEV_MEMORY_ACCEPTANCE_PROTOCOL_GUID \
    EFI_GUID (0xc5a010fe, 0x38a7, 0x4531, 0x8a, 0x4a, 0x05, 0x00, 0xd2, 0xfd, 0x16, 0x49)
...
#endif
#ifdef CONFIG_X86
extern efi_status_t efi_query_variable_store (u32 attributes, unsigned long size, bool nonblocking);
#else
...
extern void efi_systab_report_header (const efi_table_hdr_t* systab_hdr, unsigned long fw_vendor);
...
extern int efi_memattr_apply_permissions (struct mm_struct* mm, efi_memattr_perm_setter fn);
 ...
 */
#define efi_memdesc_ptr(map, desc_size, n) (efi_memory_desc_t*)((void*)(map) + ((n) * (desc_size)))

/* Iterate through an efi_memory_map */
#define for_each_efi_memory_desc_in_map(m, md)                                    \
    for ((md) = (m)->map; (md) && ((void*)(md) + (m)->desc_size) <= (m)->map_end; \
 ...
 */
#define for_each_efi_memory_desc(md) for_each_efi_memory_desc_in_map (&efi.memmap, md)

/*
 * Format an EFI memory descriptor's type and attributes to a user-provided
 * character buffer, as per snprintf(), and return the buffer.
 */
char* __init efi_md_typeattr_format (char* buf, size_t size, const efi_memory_desc_t* md);

typedef void (*efi_element_handler_t) (const char* source,
                                       const void* element_data,
                                       size_t      element_size);
extern int __init
                  parse_efi_signature_list (const char* source,
                                            const void* data,
                                            size_t      size,
...
{
    return IS_ENABLED (CONFIG_EFI_SOFT_RESERVE) && __efi_soft_reserve_enabled ();
}
...
static inline void efi_reboot (enum reboot_mode reboot_mode, const char* __unused)
{
}
...
efi_status_t efivar_get_variable (
        efi_char16_t* name, efi_guid_t* vendor, u32* attr, unsigned long* size, void* data);

efi_status_t
efivar_get_next_variable (unsigned long* name_size, efi_char16_t* name, efi_guid_t* vendor);

efi_status_t efivar_set_variable_locked (efi_char16_t* name,
                                         efi_guid_t*   vendor,
                                         u32           attr,
                                         unsigned long data_size,
                                         void*         data,
...
efi_status_t efivar_set_variable (
        efi_char16_t* name, efi_guid_t* vendor, u32 attr, unsigned long data_size, void* data);
...
extern int  efi_capsule_supported (efi_guid_t guid, u32 flags, size_t size, int* reset);

extern int  efi_capsule_update (efi_capsule_header_t* capsule, phys_addr_t* pages);
#else
...
static inline enum efi_secureboot_mode efi_get_secureboot_mode (efi_get_variable_t* get_var)
{
    ...
    size   = sizeof (secboot);
    status = get_var (L"SecureBoot", &EFI_GLOBAL_VARIABLE_GUID, NULL, &size, &secboot);
...
efi_status_t         efi_call_acpi_prm_handler (efi_status_t (__efiapi* handler_addr) (u64, void*),
...
extern struct efi_mokvar_table_entry*
efi_mokvar_entry_next (struct efi_mokvar_table_entry** mokvar_entry);
extern struct efi_mokvar_table_entry* efi_mokvar_entry_find (const char* name);
#else
static inline void efi_mokvar_table_init (void)
{
}
static inline struct efi_mokvar_table_entry*
efi_mokvar_entry_next (struct efi_mokvar_table_entry** mokvar_entry)
{
    return NULL;
}
static inline struct efi_mokvar_table_entry* efi_mokvar_entry_find (const char* name)
{
    return NULL;
}
#endif
...
static inline bool efi_config_table_is_usable (const efi_guid_t* guid, unsigned long table)
{
