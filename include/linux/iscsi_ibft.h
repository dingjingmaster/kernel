...
#else
/**
 * Linux 内核中用于保留 Intel iBFT（Intel Boot Firmware Table）表内存区域的核心函数，其作用可概括为以下几点：
 *  - iBFT 表定位与保留(通过解析 UEFI 固件 中的 iBFT 表（Intel Boot Firmware Table），确定其物理内存地址范围，并将该区域标记为 e820_type_reserved，防止内核或用户态程序覆盖)
