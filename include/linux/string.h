 ...
 */
static inline __realloc_size (2, 3) void* memdup_array_user (const void __user* src,
                                                             size_t             n,
                                                             size_t             size)
{
    size_t nbytes;
 ...
 */
static inline __realloc_size (2, 3) void* vmemdup_array_user (const void __user* src,
                                                              size_t             n,
                                                              size_t             size)
{
    size_t nbytes;
...
extern ssize_t
memory_read_from_buffer (void* to, size_t count, loff_t* ppos, const void* from, size_t available);
...
#define strtomem_pad(dest, src, pad)                                                          \
    do {                                                                                      \
        ...
        const size_t _src_len  = __builtin_object_size (src, 1);                              \
                                                                                              \
        BUILD_BUG_ON (!__builtin_constant_p (_dest_len) || _dest_len == (size_t)-1);          \
        memcpy_and_pad (dest, _dest_len, src, strnlen (src, min (_src_len, _dest_len)), pad); \
    } while (0)
...
#define memtostr(dest, src)                                                                    \
    do {                                                                                       \
        ...
        const size_t _copy_len  = min (_dest_len - 1, _src_chars);                             \
                                                                                               \
        BUILD_BUG_ON (!__builtin_constant_p (_dest_len) || !__builtin_constant_p (_src_len) || \
                      _dest_len == 0 || _dest_len == (size_t)-1 || _src_len == 0 ||            \
                      _src_len == (size_t)-1);                                                 \
