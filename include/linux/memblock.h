...
enum memblock_flags
{
    ...
    MEMBLOCK_NOMAP =
            0x4, /* don't add to kernel direct mapping。内存不参与内核直接映射，常用于预留或特殊用途 */
    MEMBLOCK_DRIVER_MANAGED =
            0x8, /* always detected via a driver。总是通过驱动程序检测和添加的内存区域，
...
/**
 * @brief 描述memblock框架中记录内存区域的集合，它维护了一个内存区域的数组，并跟踪其中区域的数量和分配的最大容量。
 *
...
struct memblock
{
    bool        bottom_up; /* is bottom up direction? 内存分配方向：是自低向高？ 还是自高向低？*/
...
int  memblock_add_node (phys_addr_t base, phys_addr_t size, int nid, enum memblock_flags flags);
...
#ifdef CONFIG_HAVE_MEMBLOCK_PHYS_MAP
static inline void __next_physmem_range (u64*                  idx,
...
#    define for_each_physmem_range(i, type, p_start, p_end)                                \
        for (i = 0, __next_physmem_range (&i, type, p_start, p_end); i != (u64)ULLONG_MAX; \
...
#define __for_each_mem_range(i, type_a, type_b, nid, flags, p_start, p_end, p_nid)        \
    for (i = 0, __next_mem_range (&i, nid, flags, type_a, type_b, p_start, p_end, p_nid); \
         i != (u64)ULLONG_MAX;                                                            \
 ...
 * @p_nid: ptr to int for nid of the range, can be %NULL
 */
#define __for_each_mem_range_rev(i, type_a, type_b, nid, flags, p_start, p_end, p_nid) \
...
#define for_each_mem_range(i, p_start, p_end)                      \
    __for_each_mem_range (i, &memblock.memory, NULL, NUMA_NO_NODE, \
                          MEMBLOCK_HOTPLUG | MEMBLOCK_DRIVER_MANAGED, p_start, p_end, NULL)
...
#define for_each_reserved_mem_range(i, p_start, p_end)                                       \
    __for_each_mem_range (i, &memblock.reserved, NULL, NUMA_NO_NODE, MEMBLOCK_NONE, p_start, \
                          p_end, NULL)
...
void __next_mem_pfn_range_in_zone (u64*           idx,
                                   struct zone*   zone,
...
#define for_each_free_mem_range(i, nid, flags, p_start, p_end, p_nid)                          \
    __for_each_mem_range (i, &memblock.memory, &memblock.reserved, nid, flags, p_start, p_end, \
                          p_nid)
...
#define for_each_free_mem_range_reverse(i, nid, flags, p_start, p_end, p_nid)                      \
    __for_each_mem_range_rev (i, &memblock.memory, &memblock.reserved, nid, flags, p_start, p_end, \
                              p_nid)
...
void* memblock_alloc_exact_nid_raw (
...
{
    return memblock_alloc_try_nid (size, align, MEMBLOCK_LOW_LIMIT, MEMBLOCK_ALLOC_ACCESSIBLE,
                                   NUMA_NO_NODE);
}

static inline void* memblock_alloc_raw (phys_addr_t size, phys_addr_t align)
{
    return memblock_alloc_try_nid_raw (size, align, MEMBLOCK_LOW_LIMIT, MEMBLOCK_ALLOC_ACCESSIBLE,
                                       NUMA_NO_NODE);
}
...
{
    return memblock_alloc_try_nid (size, align, MEMBLOCK_LOW_LIMIT, ARCH_LOW_ADDRESS_LIMIT,
                                   NUMA_NO_NODE);
}
...
#define for_each_mem_region(region)        \
    for (region = memblock.memory.regions; \
         region < (memblock.memory.regions + memblock.memory.cnt); region++)
...
#define for_each_reserved_mem_region(region) \
    for (region = memblock.reserved.regions; \
         region < (memblock.reserved.regions + memblock.reserved.cnt); region++)
