...
#define cpu_possible_mask \
    ((const struct cpumask*)&__cpu_possible_mask) // 表示系统中有多少个可以执行的CPU核心
#define cpu_online_mask \
    ((const struct cpumask*)&__cpu_online_mask)   // 表示系统中有多少个正在运行的CPU
#define cpu_enabled_mask ((const struct cpumask*)&__cpu_enabled_mask)
#define cpu_present_mask \
    ((const struct cpumask*)&__cpu_present_mask) // 表示系统中具备Online条件的核心，它们不一定都处于online，有的CPU核心可能被热插拔
#define cpu_active_mask \
    ((const struct cpumask*)&__cpu_active_mask) // 表示系统当中有多少个活跃的CPU核心
 ...
 */
static __always_inline unsigned int cpumask_first_and (const struct cpumask* srcp1,
 ...
 */
static __always_inline unsigned int cpumask_first_and_and (const struct cpumask* srcp1,
                                                           const struct cpumask* srcp2,
                                                           const struct cpumask* srcp3)
{
    return find_first_and_and_bit (cpumask_bits (srcp1), cpumask_bits (srcp2), cpumask_bits (srcp3),
                                   small_cpumask_bits);
}
...
cpumask_next_and (int n, const struct cpumask* src1p, const struct cpumask* src2p)
{
    ...
    if (n != -1)
        cpumask_check (n);
    return find_next_and_bit (cpumask_bits (src1p), cpumask_bits (src2p), small_cpumask_bits,
                              n + 1);
}
 ...
 * Return: >= nr_cpu_ids if no cpus set.
 */
static __always_inline unsigned int cpumask_any_but (const struct cpumask* mask, unsigned int cpu)
{
    unsigned int i;
 ...
 */
static __always_inline unsigned int
cpumask_any_and_but (const struct cpumask* mask1, const struct cpumask* mask2, unsigned int cpu)
{
    unsigned int i;
 ...
 */
static __always_inline unsigned int
cpumask_nth_and (unsigned int cpu, const struct cpumask* srcp1, const struct cpumask* srcp2)
{
    return find_nth_and_bit (cpumask_bits (srcp1), cpumask_bits (srcp2), small_cpumask_bits,
                             cpumask_check (cpu));
}
 ...
 */
static __always_inline unsigned int
cpumask_nth_andnot (unsigned int cpu, const struct cpumask* srcp1, const struct cpumask* srcp2)
{
    return find_nth_andnot_bit (cpumask_bits (srcp1), cpumask_bits (srcp2), small_cpumask_bits,
                                cpumask_check (cpu));
}
 ...
 * Return: >= nr_cpu_ids if such cpu doesn't exist.
 */
static __always_inline unsigned int cpumask_nth_and_andnot (unsigned int          cpu,
                                                            const struct cpumask* srcp1,
                                                            const struct cpumask* srcp2,
                                                            const struct cpumask* srcp3)
{
    return find_nth_and_andnot_bit (cpumask_bits (srcp1), cpumask_bits (srcp2),
                                    cpumask_bits (srcp3), small_cpumask_bits, cpumask_check (cpu));
}
...
cpumask_and (struct cpumask* dstp, const struct cpumask* src1p, const struct cpumask* src2p)
{
    return bitmap_and (cpumask_bits (dstp), cpumask_bits (src1p), cpumask_bits (src2p),
                       small_cpumask_bits);
}
...
cpumask_or (struct cpumask* dstp, const struct cpumask* src1p, const struct cpumask* src2p)
{
    bitmap_or (cpumask_bits (dstp), cpumask_bits (src1p), cpumask_bits (src2p), small_cpumask_bits);
}
 ...
 */
static __always_inline void
cpumask_xor (struct cpumask* dstp, const struct cpumask* src1p, const struct cpumask* src2p)
{
    bitmap_xor (cpumask_bits (dstp), cpumask_bits (src1p), cpumask_bits (src2p),
                small_cpumask_bits);
}
 ...
 */
static __always_inline bool
cpumask_andnot (struct cpumask* dstp, const struct cpumask* src1p, const struct cpumask* src2p)
{
    return bitmap_andnot (cpumask_bits (dstp), cpumask_bits (src1p), cpumask_bits (src2p),
                          small_cpumask_bits);
}
 ...
 */
static __always_inline bool cpumask_equal (const struct cpumask* src1p, const struct cpumask* src2p)
{
    return bitmap_equal (cpumask_bits (src1p), cpumask_bits (src2p), small_cpumask_bits);
}
...
{
    return bitmap_or_equal (cpumask_bits (src1p), cpumask_bits (src2p), cpumask_bits (src3p),
                            small_cpumask_bits);
}
...
static __always_inline bool cpumask_intersects (const struct cpumask* src1p,
                                                const struct cpumask* src2p)
{
    return bitmap_intersects (cpumask_bits (src1p), cpumask_bits (src2p), small_cpumask_bits);
}
...
static __always_inline bool cpumask_subset (const struct cpumask* src1p,
                                            const struct cpumask* src2p)
{
    return bitmap_subset (cpumask_bits (src1p), cpumask_bits (src2p), small_cpumask_bits);
}
 ...
 * Return: count of bits set in both *srcp1 and *srcp2
 */
static __always_inline unsigned int cpumask_weight_andnot (const struct cpumask* srcp1,
                                                           const struct cpumask* srcp2)
{
...
{
    bitmap_shift_right (cpumask_bits (dstp), cpumask_bits (srcp), n, small_cpumask_bits);
}
 ...
 */
static __always_inline void cpumask_copy (struct cpumask* dstp, const struct cpumask* srcp)
{
...
cpumask_parse_user (const char __user* buf, int len, struct cpumask* dstp)
{
 ...
 */
static __always_inline int
cpumask_parselist_user (const char __user* buf, int len, struct cpumask* dstp)
{
...
static __always_inline bool zalloc_cpumask_var_node (cpumask_var_t* mask, gfp_t flags, int node)
{
...
static __always_inline bool alloc_cpumask_var_node (cpumask_var_t* mask, gfp_t flags, int node)
{
    return true;
}
...
#define assign_cpu(cpu, mask, val) assign_bit (cpumask_check (cpu), cpumask_bits (mask), (val))
 ...
 * nothing is copied.
 */
static __always_inline ssize_t cpumap_print_to_pagebuf (bool                  list,
                                                        char*                 buf,
 ...
 * terminating '\0'.
 */
static __always_inline ssize_t cpumap_print_bitmask_to_buf (char*                 buf,
                                                            const struct cpumask* mask,
                                                            loff_t                off,
                                                            size_t                count)
{
    return bitmap_print_bitmask_to_buf (buf, cpumask_bits (mask), nr_cpu_ids, off, count) - 1;
}
 ...
 * terminating '\0'.
 */
static __always_inline ssize_t cpumap_print_list_to_buf (char*                 buf,
                                                         const struct cpumask* mask,
                                                         loff_t                off,
                                                         size_t                count)
{
    return bitmap_print_list_to_buf (buf, cpumask_bits (mask), nr_cpu_ids, off, count) - 1;
}
