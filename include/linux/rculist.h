...
#define __list_check_srcu(cond) ({ RCU_LOCKDEP_WARN (!(cond), "RCU-list traversed without holding the required lock!"); })
#else
...
#define list_for_each_entry_rcu(pos, head, member, cond...)                                                                       \
    for (__list_check_rcu (dummy, ##cond, 0), pos = list_entry_rcu ((head)->next, typeof (*pos), member); &pos->member != (head); \
...
#define list_for_each_entry_continue_rcu(pos, head, member) \
    for (pos = list_entry_rcu (pos->member.next, typeof (*pos), member); &pos->member != (head); pos = list_entry_rcu (pos->member.next, typeof (*pos), member))
...
#define hlist_for_each_entry_rcu(pos, head, member, cond...)                                                                                       \
    for (__list_check_rcu (dummy, ##cond, 0), pos = hlist_entry_safe (rcu_dereference_raw (hlist_first_rcu (head)), typeof (*(pos)), member); pos; \
...
#define hlist_for_each_entry_srcu(pos, head, member, cond)                                                                              \
    for (__list_check_srcu (cond), pos = hlist_entry_safe (rcu_dereference_raw (hlist_first_rcu (head)), typeof (*(pos)), member); pos; \
         pos = hlist_entry_safe (rcu_dereference_raw (hlist_next_rcu (&(pos)->member)), typeof (*(pos)), member))
