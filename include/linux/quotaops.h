...
static inline bool is_quota_modification (struct mnt_idmap* idmap, struct inode* inode, struct iattr* ia)
{
    return ((ia->ia_valid & ATTR_SIZE) || i_uid_needs_update (idmap, ia, inode) || i_gid_needs_update (idmap, ia, inode));
}

#if defined(CONFIG_QUOTA)

#define quota_error(sb, fmt, args...) __quota_error ((sb), __func__, fmt, ##args)

extern __printf (3, 4) void __quota_error (struct super_block* sb, const char* func, const char* fmt, ...);
...
int                              dquot_get_dqblk (struct super_block* sb, struct kqid id, struct qc_dqblk* di);
int                              dquot_get_next_dqblk (struct super_block* sb, struct kqid* id, struct qc_dqblk* di);
int                              dquot_set_dqblk (struct super_block* sb, struct kqid id, struct qc_dqblk* di);
...
{
    return sb_dqopt (sb)->flags & dquot_state_flag (DQUOT_USAGE_ENABLED, type);
}

static inline bool sb_has_quota_limits_enabled (struct super_block* sb, int type)
{
    return sb_dqopt (sb)->flags & dquot_state_flag (DQUOT_LIMITS_ENABLED, type);
}
...
{
    return sb_has_quota_loaded (sb, type) && !sb_has_quota_suspended (sb, type);
}
...
static inline int dquot_transfer (struct mnt_idmap* idmap, struct inode* inode, struct iattr* iattr)
{
    return 0;
}

static inline int __dquot_alloc_space (struct inode* inode, qsize_t number, int flags)
{
...
static inline void __dquot_free_space (struct inode* inode, qsize_t number, int flags)
{
    if (!(flags & DQUOT_SPACE_RESERVE))
...
static inline int dquot_reclaim_space_nodirty (struct inode* inode, qsize_t number)
{
    inode_sub_bytes (inode, number);
    return 0;
}

static inline int dquot_disable (struct super_block* sb, int type, unsigned int flags)
{
    return 0;
}
...
{
    return __dquot_alloc_space (inode, nr << inode->i_blkbits, DQUOT_SPACE_WARN | DQUOT_SPACE_RESERVE);
}
...
static inline void dquot_release_reservation_block (struct inode* inode, qsize_t nr)
{
