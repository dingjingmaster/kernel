...
#define RCU_LOCKDEP_WARN(c, s)                                                                  \
    do {                                                                                        \
        ...
        if (debug_lockdep_rcu_enabled () && (c) && debug_lockdep_rcu_enabled () && !__warned) { \
            __warned = true;                                                                    \
...
{
    return debug_lockdep_rcu_enabled () && (c || !rcu_is_watching () || !rcu_lockdep_current_cpu_online ()) && debug_lockdep_rcu_enabled ();
}
 ...
 */
#define lockdep_assert_in_rcu_read_lock() WARN_ON_ONCE (lockdep_assert_rcu_helper (!lock_is_held (&rcu_lock_map)))
 ...
 */
#define lockdep_assert_in_rcu_read_lock_bh() WARN_ON_ONCE (lockdep_assert_rcu_helper (!lock_is_held (&rcu_bh_lock_map)))
 ...
 */
#define lockdep_assert_in_rcu_read_lock_sched() WARN_ON_ONCE (lockdep_assert_rcu_helper (!lock_is_held (&rcu_sched_lock_map)))
...
#define lockdep_assert_in_rcu_reader()                                                                                                                \
    WARN_ON_ONCE (lockdep_assert_rcu_helper (!lock_is_held (&rcu_lock_map) && !lock_is_held (&rcu_bh_lock_map) && !lock_is_held (&rcu_sched_lock_map) \
                                             && preemptible ()))
