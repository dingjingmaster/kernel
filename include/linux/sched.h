 ...
 */
#define is_special_task_state(state) ((state) & (__TASK_STOPPED | __TASK_TRACED | TASK_PARKED | TASK_DEAD | TASK_FROZEN))
...
struct task_struct
{
    ...
    int              nr_cpus_allowed; //
    const cpumask_t* cpus_ptr;        // 允许进程在哪个CPU上运行
    ...
    // 这两个指针指向内存描述符、进程：mm和active_mm指向同一个内存描述符
    // 内核线程 mm 是一个空指针，运行时候，active_mm指向从进程借用内存描述符
    struct mm_struct*     mm;        //
    ...
    struct task_struct __rcu* parent; // 指向父进程，如果被进程跟踪则为跟踪进程ID，否则与 real_parent 一样
     ...
     */
    char              comm[TASK_COMM_LEN]; // 应用层进程名称
    ...
    struct fs_struct*    fs; // 指向文件系统信息。主要进程的根目录和当前工作目录

    /* Open file information: */
    struct files_struct* files; // 打开文件表有哪些
...
static inline unsigned int __task_state_index (unsigned int tsk_state, unsigned int tsk_exit_state)
{
 ...
 */
#    define PREEMPT_LOCK_RESCHED_OFFSETS (PREEMPT_LOCK_OFFSET + (1U << MIGHT_RESCHED_RCU_SHIFT))
#endif
...
extern int  sched_core_share_pid (unsigned int cmd, pid_t pid, enum pid_type type, unsigned long uaddr);
