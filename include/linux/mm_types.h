...
struct vm_area_struct
{
    ...
    union
    {
        struct
        {
            ...
            unsigned long vm_end;   // 虚拟内存空间末尾地址
        };
    ...
    struct
    {
        struct rb_node rb; // 将VMA作为节点加入红黑树中
...
/**
 * struct mm_struct 是 Linux 内核中用于描述进程虚拟地址空间的核心数据结构，其作用可概括为以下几个方面：
 *   1. 描述进程虚拟地址空间
 *      - 内存区域（VMA）管理：通过 mmap 链表和红黑树（mm_rb）组织所有虚拟内存区域（vm_area_struct），描述代码段、数据段、堆栈、共享映射区等逻辑分区
 *      - 地址空间布局：记录内存映射的基地址（mmap_base）、最高虚拟地址（highest_vm_end）等参数，定义用户空间的地址范围（如 3GB）
 *      - 页表管理：通过 pgd 指针指向进程的页目录，实现虚拟地址到物理地址的映射
 *   2 进程间地址空间隔离与共享
 *      - 隔离性：每个进程拥有独立的 mm_struct，确保用户空间数据互不干扰
 *      - 共享性：线程（轻量级进程）共享所属进程的 mm_struct，实现资源共享
 *      - 内核空间共享：所有进程共享同一份内核地址空间（1GB），通过权限控制保护内核数据
 *   3 内存操作与同步
 *      - 内存分配：提供 get_unmapped_area 函数接口，用于分配未映射的虚拟地址区间
 *      - 同步机制：通过 mmap_sem 信号量保护内存区域的并发访问，防止竞态条件
 *      - 缓存优化：维护 mmap_cache 记录最近访问的虚拟内存区域，加速地址空间查询
 *   4. 引用计数与生命周期管理
 *      - mm_users：记录共享该地址空间的进程数（如多线程场景）
 *      - mm_count：记录 mm_struct 的主引用计数，当计数为 0 时释放内存
 *      - 生命周期：与进程的 task_struct 绑定，进程退出时自动释放相关资源
 *   5. 与进程控制块（task_struct）的关联
 *      - 每个 task_struct 包含指向 mm_struct 的指针（mm），用于访问进程内存信息
 *      - 内核线程（kernel_thread）无独立 mm_struct，复用父进程的地址空间
 */
struct mm_struct
{
    struct
    {
...
#endif
        int                 map_count;       /* 内存描述符的引用计数 number of VMAs */
        ...
        /**
         * arg_start/arg_end：参数字符串的起始地址和结束地址
...
static inline void
vma_iter_init (struct vma_iterator* vmi, struct mm_struct* mm, unsigned long addr)
{
...
#define VM_FAULT_RESULT_TRACE                                                             \
    {VM_FAULT_OOM, "OOM"}, {VM_FAULT_SIGBUS, "SIGBUS"}, {VM_FAULT_MAJOR, "MAJOR"},        \
            {VM_FAULT_HWPOISON, "HWPOISON"}, {VM_FAULT_HWPOISON_LARGE, "HWPOISON_LARGE"}, \
            {VM_FAULT_SIGSEGV, "SIGSEGV"}, {VM_FAULT_NOPAGE, "NOPAGE"},                   \
            {VM_FAULT_LOCKED, "LOCKED"}, {VM_FAULT_RETRY, "RETRY"},                       \
            {VM_FAULT_FALLBACK, "FALLBACK"}, {VM_FAULT_DONE_COW, "DONE_COW"},             \
            {VM_FAULT_NEEDDSYNC, "NEEDDSYNC"}, {VM_FAULT_COMPLETED, "COMPLETED"}
...
struct vm_special_mapping
{
    ...
    void (*close) (const struct vm_special_mapping* sm, struct vm_area_struct* vma);
};
