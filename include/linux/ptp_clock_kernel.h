...
struct ptp_clock_info
{
    ...
    int (*getcyclesx64) (struct ptp_clock_info* ptp, struct timespec64* ts, struct ptp_system_timestamp* sts);
    int (*getcrosscycles) (struct ptp_clock_info* ptp, struct system_device_crosststamp* cts);
...
#else
static inline struct ptp_clock* ptp_clock_register (struct ptp_clock_info* info, struct device* parent)
{
    return NULL;
}
...
static inline void ptp_clock_event (struct ptp_clock* ptp, struct ptp_clock_event* event)
{
}
...
static inline int ptp_find_pin (struct ptp_clock* ptp, enum ptp_pin_function func, unsigned int chan)
{
    return -1;
}
static inline int ptp_find_pin_unlocked (struct ptp_clock* ptp, enum ptp_pin_function func, unsigned int chan)
{
    return -1;
}
static inline int ptp_schedule_worker (struct ptp_clock* ptp, unsigned long delay)
{
    return -EOPNOTSUPP;
}
...
static inline ktime_t ptp_convert_timestamp (const ktime_t* hwtstamp, int vclock_index)
{
    return 0;
}
...
static inline void ptp_read_system_prets (struct ptp_system_timestamp* sts)
{
    if (sts) {
        switch (sts->clockid) {
        case CLOCK_REALTIME: ktime_get_real_ts64 (&sts->pre_ts); break;
        case CLOCK_MONOTONIC: ktime_get_ts64 (&sts->pre_ts); break;
        case CLOCK_MONOTONIC_RAW: ktime_get_raw_ts64 (&sts->pre_ts); break;
        default: break;
        }
    }
}
...
static inline void ptp_read_system_postts (struct ptp_system_timestamp* sts)
{
    if (sts) {
        switch (sts->clockid) {
        case CLOCK_REALTIME: ktime_get_real_ts64 (&sts->post_ts); break;
        case CLOCK_MONOTONIC: ktime_get_ts64 (&sts->post_ts); break;
        case CLOCK_MONOTONIC_RAW: ktime_get_raw_ts64 (&sts->post_ts); break;
        default: break;
        }
    }
}
