...
extern ssize_t
__modver_version_show (struct module_attribute*, struct module_kobject*, char*);
...
#    define MODULE_VERSION(_version)                                                     \
        ...
        static struct module_version_attribute __modver_attr __used __section ("__"      \
                                                                               "modver") \
...
#    define mod_mem_type_is_init(type) \
        ((type) == MOD_INIT_TEXT || (type) == MOD_INIT_DATA || (type) == MOD_INIT_RODATA)

#    define mod_mem_type_is_core(type) (!mod_mem_type_is_init (type))

#    define mod_mem_type_is_text(type) ((type) == MOD_TEXT || (type) == MOD_INIT_TEXT)

#    define mod_mem_type_is_data(type) (!mod_mem_type_is_text (type))

#    define mod_mem_type_is_core_data(type) \
        (mod_mem_type_is_core (type) && mod_mem_type_is_data (type))
...
static inline bool within_module_core (unsigned long addr, const struct module* mod)
{
...
extern void __noreturn __module_put_and_kthread_exit (struct module* mod, long code);
...
/* Dereference module function descriptor */
static inline void* dereference_module_function_descriptor (struct module* mod, void* ptr)
{
    return ptr;
}
...
#ifdef CONFIG_GENERIC_BUG
void module_bug_finalize (const Elf_Ehdr*, const Elf_Shdr*, struct module*);
void module_bug_cleanup (struct module*);

#else  /* !CONFIG_GENERIC_BUG */

static inline void
module_bug_finalize (const Elf_Ehdr* hdr, const Elf_Shdr* sechdrs, struct module* mod)
{
}
...
static inline int module_kallsyms_on_each_symbol (
        const char* modname, int (*fn) (void*, const char*, unsigned long), void* data)
{
    return -EOPNOTSUPP;
}
