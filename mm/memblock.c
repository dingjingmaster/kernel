...
// __initdata_memblock，属性的作用是将这个变量放置在内核的初始化数据区
 ...
 * Found address on success, 0 on failure.
 */
static phys_addr_t __init_memblock __memblock_find_range_top_down (phys_addr_t         start,
                                                                   phys_addr_t         end,
                                                                   phys_addr_t         size,
                                                                   phys_addr_t         align,
                                                                   int                 nid,
                                                  ...
                                                  phys_addr_t           new_area_size)
{
    ...
    if (use_slab) {
        ...
        addr = memblock_find_in_range (new_area_start + new_area_size, memblock.current_limit,
                                       new_alloc_size, PAGE_SIZE);
        if (!addr && new_area_size)
            addr = memblock_find_in_range (0, min (new_area_start, memblock.current_limit),
                                           new_alloc_size, PAGE_SIZE);
    ...
    memblock_dbg ("memblock: %s is doubled to %ld at [%pa-%pa]", type->name, type->max * 2, &addr,
                  &new_end);
 ...
 *
 * 在 @type 中添加新的 memblock 区域 [@base，@base + @size)。
 * 新区域允许与现有区域重叠--重叠不会影响已存在的区域。
 * 添加后，保证 @type 最小（所有相邻的兼容区域都被合并）。
 *
 * Return:
    ...
    *start_rgn = *end_rgn = 0;
    ...
    for_each_memblock_type (idx, type, rgn)
    {
        ...
        if (rbase < base) {
            ...
            memblock_insert_region (type, idx, rbase, base - rbase, memblock_get_region_node (rgn),
                                    rgn->flags);
        } else if (rend > end) {
            /*
            ...
            memblock_insert_region (type, idx--, rbase, end - rbase, memblock_get_region_node (rgn),
                                    rgn->flags);
        } else {
...
static int __init_memblock memblock_setclr_flag (
        struct memblock_type* type, phys_addr_t base, phys_addr_t size, int set, int flag)
{
...
static bool
should_skip_region (struct memblock_type* type, struct memblock_region* m, int nid, int flags)
{
 ...
 */
void __init_memblock __next_mem_pfn_range (
        int* idx, int nid, unsigned long* out_start_pfn, unsigned long* out_end_pfn, int* out_nid)
{
 ...
 * 0 on success, -errno on failure.
 */
int __init_memblock memblock_set_node (phys_addr_t           base,
                                       phys_addr_t           size,
 ...
 */
void __init_memblock __next_mem_pfn_range_in_zone (u64*           idx,
                                                   struct zone*   zone,
                                                   ...
                                                   unsigned long* out_epfn)
{
    ...
    __next_mem_range (idx, zone_nid, MEMBLOCK_NONE, &memblock.memory, &memblock.reserved, &spa,
                      &epa, NULL);
                                              ...
                                              phys_addr_t end)
{
    memblock_dbg ("%s: %llu bytes align=0x%llx from=%pa max_addr=%pa %pS\n", __func__, (u64)size,
                  (u64)align, &start, &end, (void*)_RET_IP_);
 ...
 */
static void* __init memblock_alloc_internal (phys_addr_t size,
                                             phys_addr_t align,
...
void* __init memblock_alloc_exact_nid_raw (
        phys_addr_t size, phys_addr_t align, phys_addr_t min_addr, phys_addr_t max_addr, int nid)
{
    memblock_dbg ("%s: %llu bytes align=0x%llx nid=%d from=%pa max_addr=%pa %pS\n", __func__,
                  (u64)size, (u64)align, nid, &min_addr, &max_addr, (void*)_RET_IP_);
...
static void __init reserved_mem_add (phys_addr_t start, phys_addr_t size, const char* name)
{
...
static const char* const flagname[] = {
        [ilog2 (MEMBLOCK_HOTPLUG)] = "HOTPLUG",     [ilog2 (MEMBLOCK_MIRROR)] = "MIRROR",
        [ilog2 (MEMBLOCK_NOMAP)] = "NOMAP",         [ilog2 (MEMBLOCK_DRIVER_MANAGED)] = "DRV_MNG",
