...
void __init vfs_caches_init(void)
{
	...
	// 在 start_kernel 过程中，vfs_caches_init 及其子函数（如 mnt_init）所执行的初始化和挂载，确实只是一个纯粹的虚拟根目录
	// mnt_init 中执行的“挂载”动作，其目标是一个名为 rootfs 的特殊文件系统:
	// 1. 它是虚拟的：它完全驻留在内存中(基于 ramfs 或 tmpfs)
	// 2. 它是空的：在刚创建的那一刻，这个根目录下没有任何文件，只有一个孤零零的目录项(dentry)
	// 3. 它是不可见的：用户在系统启动后通常看不到这个最初的 rootfs，因为它随后会被“真实根”覆盖
	//
	// 内核在启动阶段必须先建立这个虚拟根，主要有三个目的：
    // 1. 提供路径解析的基础：Linux 的所有文件操作（甚至是内核内部的）都必须基于“路径”。没有这个虚拟根，内核就无法调用 open()、mount() 等函数，因为这些函数需要一个起始点
	// 2. 作为挂载“跳板”：内核需要一个地方来挂载你的 initramfs。initramfs 会被解压到这个虚拟的 rootfs 中
	// 3. 存放初始化工具：在磁盘驱动还没加载好之前，内核利用这个虚拟根里的脚本（如 /init）来加载 NVMe、USB 或 RAID 驱动
	// 
	// start_kernel 结束后，内核会启动 rest_init 并最终进入用户态初始化。此时会发生 根切换（Root Switching）
	// 1. 挂载真实磁盘：内核根据启动参数（如 root=/dev/nvme0n1p2）将真实的磁盘分区挂载到虚拟根下的某个目录（例如 /root）
	// 2. 切换视图：通过 pivot_root 或 2026 年更常用的 move_mount 机制，将真实磁盘挂载点移动到当前命名空间的 / 位置
	// 3. 释放虚拟根：原本的 rootfs 会被“隐藏”或释放
}
