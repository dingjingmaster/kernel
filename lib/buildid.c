...
static void freader_init_from_file (struct freader* r, void* buf, u32 buf_sz, struct file* file, bool may_fault)
{
    memset (r, 0, sizeof (*r));
    r->buf       = buf;
    r->buf_sz    = buf_sz;
...
static int freader_get_folio (struct freader* r, loff_t file_off)
{
    ...
    if (r->folio && file_off >= r->folio_off && file_off < r->folio_off + folio_size (r->folio))
        return 0;
    ...
    if (r->may_fault && (IS_ERR (r->folio) || !folio_test_uptodate (r->folio))) {
        ...
        r->folio = read_cache_folio (r->file->f_mapping, file_off >> PAGE_SHIFT, NULL, r->file);
...
static int parse_build_id (struct freader* r, unsigned char* build_id, __u32* size, loff_t note_off, Elf32_Word note_size)
{
    ...
    while (note_end - note_off > sizeof (Elf32_Nhdr) + note_name_sz) {
        ...
        if (nhdr->n_type == BUILD_ID && name_sz == note_name_sz && memcmp (nhdr + 1, note_name, note_name_sz) == 0 && desc_sz > 0
            && desc_sz <= BUILD_ID_SIZE_MAX) {
...
static int get_build_id_32 (struct freader* r, unsigned char* build_id, __u32* size)
{
    ...
    for (i = 0; i < phnum; ++i) {
        ...
        if (phdr->p_type == PT_NOTE && !parse_build_id (r, build_id, size, READ_ONCE (phdr->p_offset), READ_ONCE (phdr->p_filesz)))
            return 0;
    }
    return -EINVAL;
}
...
static int __build_id_parse (struct vm_area_struct* vma, unsigned char* build_id, __u32* size, bool may_fault)
{
    const Elf32_Ehdr* ehdr;
    struct freader    r;
